---
title: Penghu island ferry services
summary: Public and visitor ferry services linking Magong with Penghu's inhabited islands, the southern islands and Taiwan, with dated schedules, fares and weather limits.
updated: 2026-08-26
aliases:
  - Penghu ferries
  - 澎湖交通船
  - 澎湖離島交通
facts:
  - label: Core south-sea service
    value: Magong–Wangan–Qimei
    source: phpto-timetable
  - label: Retrieved schedule window
    value: 2026-08-26 to 2026-09-01
    source: phpto-timetable
  - label: Magong–Qimei adult fare in the official visitor guide
    value: NT$437
    source: penghu-nsa-island-hopping
specs:
  - label: Magong–Wangan fare in the official visitor guide
    value: 272
    unit: NT$
    source: penghu-nsa-island-hopping
  - label: Summer Budai–Magong light-ferry crossing
    value: 90
    unit: minutes (approximate)
    source: penghu-nsa-ship-info
sources:
  - id: penghu-wheel-about
    title: About Penghu Wheel
    titleOriginal: 關於澎湖輪
    publisher: 台灣航業股份有限公司 (Taiwan Navigation Co., Ltd.)
    url: "https://tnc-kao.com.tw/about/about"
    accessed: 2026-08-29
    kind: primary
    lang: zh-Hant
    note: "Operator's own booking-system 'about' page. Gives the 5 September 2023 maiden voyage, the May 2021 tender selection, and full as-delivered specifications."
  - id: ltn-penghu-wheel-contract
    title: New Penghu ferry contracted to a Japanese shipyard
    titleOriginal: 新台澎輪委託日本內海船廠建造
    publisher: 自由時報 (Liberty Times)
    url: "https://news.ltn.com.tw/news/life/breakingnews/3982295"
    accessed: 2026-08-29
    kind: secondary
    lang: zh-Hant
    note: "News report on the pre-delivery construction contract, giving a 9,980-ton figure and other specifications that differ slightly from the operator's own as-delivered figures — reported as a conflict, not resolved. The article calls the vessel '新台澎輪,' a name not used elsewhere."
  - id: zhwiki-taihua
    title: Taihua Ferry
    titleOriginal: 臺華輪
    publisher: Wikipedia contributors (維基百科)
    url: "https://zh.wikipedia.org/zh-tw/臺華輪"
    accessed: 2026-08-29
    kind: secondary
    lang: zh-Hant
    note: "Lead-only encyclopedia article, footnoted to CNA, NowNews, UDN and other press but not independently verified against those primaries in this pass. Supports the September 1989 completion, 34 years of service and 23 August 2023 final sailing."
  - id: phpto-timetable
    title: Public ferry timetable
    titleOriginal: 交通船時刻表
    publisher: 澎湖縣政府公共車船管理處 (Penghu County Public Vehicle and Ship Management Office)
    url: "https://www.phpto.gov.tw/home.jsp?id=106"
    accessed: 2026-08-26
    kind: primary
    lang: zh-Hant
    note: "The live official timetable page fetched on 26 August 2026. Its displayed query covers 26 August–1 September 2026 and lists South Sea services, vessel names, ticket points, weather-stop rules and the page update date of 18 August 2026."
  - id: phpto-ticket-guidance
    title: Public ferry ticket-purchase guidance
    titleOriginal: 澎湖縣政府公共車船管理處交通船船票購買須知
    publisher: 澎湖縣政府公共車船管理處 (Penghu County Public Vehicle and Ship Management Office)
    url: "https://www.penghu.gov.tw/userfiles/03/files/%E6%BE%8E%E6%B9%96%E7%B8%A3%E6%94%BF%E5%BA%9C%E5%85%AC%E5%85%B1%E8%BB%8A%E8%88%B9%E7%AE%A1%E7%90%86%E8%99%95%E4%BA%A4%E9%80%9A%E8%88%B9%E8%88%AA%E7%8F%AD%E8%B3%BC%E7%A5%A8%E9%A0%88%E7%9F%A5114_07_19.pdf"
    accessed: 2026-08-26
    kind: primary
    lang: zh-Hant
    note: "The full one-page official guidance, effective 19 July 2025. It describes fixed and special services, ticket-sales windows, online reservations, wind thresholds and refunds."
  - id: penghu-nsa-island-hopping
    title: South Sea ferry services and island-hopping transport
    titleOriginal: 南海船班及航空
    publisher: 交通部觀光署澎湖國家風景區管理處 (Penghu National Scenic Area Administration)
    url: "https://www.penghu-nsa.gov.tw/ChiHoOneLer/transport/Traffic/IslandHoppingTraffic/IHT02.htm"
    accessed: 2026-08-26
    kind: primary
    lang: zh-Hant
    note: "The full official visitor-information page, updated 14 August 2026. It identifies the North, East and South departure centres, publishes island-service fare values and links readers to the operating authorities for current schedules."
  - id: penghu-nsa-ship-info
    title: Ferry information for reaching Penghu
    titleOriginal: 輪船資訊
    publisher: 交通部觀光署澎湖國家風景區管理處 (Penghu National Scenic Area Administration)
    url: "https://www.penghu-nsa.gov.tw/ChiHoOneLer/transport/Traffic/Traffic/ship01.htm"
    accessed: 2026-08-26
    kind: primary
    lang: zh-Hant
    note: "The full official visitor-information page, updated 14 August 2026. It describes seasonal Taiwan–Penghu sea access, published approximate crossing time and port pairs, and lists shipping operators and vessel families."
---

Penghu’s ferry network is an island service, not one route. The official visitor
information divides island-hopping departures between the North Sea Visitor
Centre at Chikan, the East Sea Visitor Centre at Qitou and the South Sea Visitor
Centre at Magong’s Third Fishing Port; it warns that schedules change with the
season and weather.[^penghu-nsa-island-hopping] This page therefore separates
the county’s public transport boats from private island-hopping packages and
keeps every date attached to the schedule it describes.

## The county’s South Sea public service

The Public Vehicle and Ship Management Office’s live timetable displayed a
query for 26 August–1 September 2026 when fetched on 26 August.[^phpto-timetable] It lists
南海之星2號 (Nanhai Star No. 2), 海有1, the Friday special boat 亞曼尼, and
overnight 七美 sailings, so the week is not a single repeating table.[^phpto-timetable]
The regular pattern shown for 26 August, 29 August, 30 August and 1 September
is Magong 09:30, Wangan 10:20, Wangan 10:50, Qimei 11:35, Qimei 13:30,
Wangan 14:15, Wangan 14:30 and Magong 15:20.[^phpto-timetable] On 27 August,
the same page instead shows a service continuing from Qimei to Kaohsiung,
with Qimei at 12:30 and Kaohsiung at 15:10.[^phpto-timetable] Its 28 August
listing includes a Kaohsiung departure at 09:50, Qimei at 12:40, Wangan at
14:50 and Magong at 16:20.[^phpto-timetable]

These are dated departures, not a promise that a boat will run regardless of
conditions. The live page says the Magong–Wangan–Qimei service stops at wave
height of 3 metres or wind above 15.6 m/s, the Qimei–Kaohsiung service stops
above 13.6 m/s, and overnight or Friday-special services stop above 10.5
m/s; the captain may also decide from sea conditions.[^phpto-timetable] The
same page notes that the boats carry both passengers and cargo, so heavy
loading can delay departure, and that charter or weather-affected services may
be adjusted.[^phpto-timetable]

## Tickets, reservations and island fares

The county’s ticket guidance, effective from 19 July 2025, says that outbound
travel to Wangan or Qimei is sold in a queue rather than by reservation, with
priority for registered residents and students when seats are limited.[^phpto-ticket-guidance]
Return reservations are accepted online after the timetable is announced; the
guidance allocates 60 online places per sailing, 30 for Wangan and 30 for
Qimei, with a maximum of four tickets per person.[^phpto-ticket-guidance]
Online purchase closes at 21:00 on the day before sailing, and a mobile QR
ticket plus identity document can be used for boarding.[^phpto-ticket-guidance]
The ticket guidance says a ticket is valid only for its date and sailing, with
a 10% refund fee before departure and no fee when a sailing is cancelled for
typhoon or other force majeure, subject to its stated refund window.[^phpto-ticket-guidance]

The National Scenic Area Administration’s island-transport table gives the
following published one-way values. It directs readers to the responsible
authority for the live operating rules and timetable.[^penghu-nsa-island-hopping]

| Service | Published one-way fare | Authority named for the timetable |
| --- | ---: | --- |
| Magong–Tongpan | NT$120 | Magong City Office[^penghu-nsa-island-hopping] |
| Magong–Hujing | NT$120 | Magong City Office[^penghu-nsa-island-hopping] |
| Magong–Wangan | NT$272 | Public Vehicle and Ship Management Office[^penghu-nsa-island-hopping] |
| Magong–Qimei | NT$437 | Public Vehicle and Ship Management Office[^penghu-nsa-island-hopping] |
| Wangan–Qimei | NT$167 | Public Vehicle and Ship Management Office[^penghu-nsa-island-hopping] |
| Magong–Southern Four Islands | NT$460 | Wangan Township Office[^penghu-nsa-island-hopping] |

The same guide lists a separate Southern Four Islands entry for Magong–Wangan
at NT$300, alongside the NT$460 Magong–Southern Four Islands value.[^penghu-nsa-island-hopping]
Those entries describe different service categories in the guide; they should
not be silently collapsed into one fare. The page links to Magong’s Tongpan and
Hujing timetable and to Wangan Township’s Southern Four Islands timetable, but
the two linked municipal pages timed out during this verification, so current
departure times for those services remain TBC.[^penghu-nsa-island-hopping]

## Reaching Penghu from Taiwan

The National Scenic Area Administration describes a seasonal mainland-island
pattern. Light passenger ferries operate in summer between Chiayi’s Budai Port
and Magong, with an approximate crossing time of 90 minutes; in winter, it says
the Taiwan Navigation Penghu ferry is the remaining regular sea option between
Kaohsiung’s Xinbin Pier and Magong, taking about four hours, with summer night
voyages also possible.[^penghu-nsa-ship-info] Its port table also lists Budai–Longmen,
Kaohsiung–Qimei/Wangan/Magong and Tainan–Dongji links.[^penghu-nsa-ship-info]
The same official page names Taiwan Navigation, Hai An Hai You, Manten Star,
Kaixuan, Bailey and Yongjie as the operator families associated with the listed
routes and vessels.[^penghu-nsa-ship-info]

The page itself says that mainland sailing changes with season and weather, so
the operator list is an orientation index rather than a live departure board.[^penghu-nsa-ship-info]
For the same reason, this guide does not invent a year-round frequency, vessel
capacity, or a guaranteed ferry-to-airport connection. Those fields need a
dated operator timetable or a live official query before they can be promoted.

## The Kaohsiung ferry's vessel history

The "Taiwan Navigation Penghu ferry" named above as the winter mainland-island
option is 澎湖輪 (Penghu Wheel), which made its maiden voyage from Kaohsiung to
Magong on 5 September 2023 after Taiwan Navigation (台航) was selected by
MOTC in a May 2021 tender for the route's long-term operation; the vessel was
built by Naikai Zosen in Japan and carries 600 passengers on a 9,932-ton hull,
119.99 m long, with capacity for 4 tour buses, 82 cars and 10 twenty-foot
containers.[^penghu-wheel-about] A pre-delivery news report gave slightly
different construction-contract specifications — 9,980 tons and a
600-plus-passenger figure that included a stated 300-berth sleeping
section — which this page reports alongside the operator's own as-delivered
figures rather than treating as the same measurement.[^ltn-penghu-wheel-contract]

Penghu Wheel replaced 臺華輪 (Taihua), which had served the Kaohsiung–Magong
route for 34 years after completion in Nagasaki in September 1989, making its
final sailing on 23 August 2023 before being sent to Bangladesh for
scrapping.[^zhwiki-taihua]

## What remains date-sensitive

The county’s live page is the source for the displayed 26 August–1 September
week, while the ticket guidance is an effective-date document from July 2025;
neither should be read as a permanent timetable.[^phpto-timetable][^phpto-ticket-guidance]
The live page and the older ticket PDF also publish slightly different wind
threshold values: the live page gives 15.6, 13.6 and 10.5 m/s for its named
service groups, while the PDF’s table displays 15.5, 10, 10 and 13.5 m/s across
its service columns.[^phpto-timetable][^phpto-ticket-guidance] The headings in
the PDF’s extracted text do not expose a clean one-to-one mapping for every
column, so the values are reported as a dated source conflict rather than
reconciled by guesswork.
