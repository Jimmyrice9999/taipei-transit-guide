---
title: "F913 / F913"
summary: "New Taipei route: F913 (F913)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F913 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 83 stops from 國泰醫院(建成路), past 崇義高中(忠孝東路), then 開山宮, then 民族六街口, then 北峰里(中興路), then 福安宮, returning to 國泰醫院(建成路).[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 汐止區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F913、F913延忠三街.[^tdx-bus] The committed record carries 2 direction/variant sequences, 83 distinct stops in total: 77 stops (outbound, F913); 83 stops (outbound, F913延忠三街).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:25 to 18:45; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “國泰醫院(建成路)-國泰醫院(建成路)”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0625-1845；返程：0625-1845; Holidays: 去程：0625-1845；返程：0625-1845.[^ntpc-bus-route-data]
