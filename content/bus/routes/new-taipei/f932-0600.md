---
title: "F932-0600 / F932-0600"
summary: "New Taipei route: F932-0600 (F932-0600)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F932-0600 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 5 stops from 金山區公所, past 重和里朝天宮, then 六股中路, then 磺溪中橋, returning to 金山區公所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 金山區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 5 distinct stops in total: 5 stops (outbound).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data] This is one of 7 separately catalogued F932 departure-time instances in the committed TDX layer (F932-0600, F932-0630, F932-0715, F932-0820, F932-0950, F932-1110 and F932-1630), each pairing a distinct stop pattern to a single scheduled trip rather than appearing as timetable rows under one shared route record.[^tdx-bus]
...
The committed TDX snapshot carries 1 directional or variant stop sequence for F932-0600; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F932-0600 (5 stops):** Jinshan District Office -> Chonghe Village Chaotien Temple -> Liuguzhong Rd -> Huangxizhong Bridge -> Jinshan District Office[^tdx-bus]
