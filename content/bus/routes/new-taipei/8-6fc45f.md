...
New Taipei City Department of Transportation's full 一般公車 dataset files 8 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 23 stops from 捷運新店站(新店路), past 溪頭, then 下城社區, then 十四份, then 安和路二段, then 新和國小, then 景新街口, ending at 捷運景安站.[^tdx-bus] 6 confirmed interchanges are recorded in the curated stop-ID join set: Xindian, Xindian District Office, Ankang, Sunshine Sports Park, Xinhe Elementary School and Jingan.[^tdx-bus] The route runs 2 subroutes under a single operator, 臺北客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 53 distinct stops in total: 23 stops (outbound); 30 stops (return).[^tdx-bus] The published fare is charged over 1 segments; the published route length is 18.3 km; weekday service runs from 06:00 to 21:00; holiday service instead runs from 07:00 to 20:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “捷運新店站-捷運景安站”; published distance 18.3 km; service span Weekdays: 去程：0600-2100；返程：0600-2100; Holidays: 去程：0700-2000；返程：0700-2000; fare 一段票 / 1 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 8; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**8 (23 stops):** MRT Xindian Sta -> Xitou -> Dapingding -> Dingcheng -> Mansion of General Hu-Lien -> Xiacheng Community -> Anhua Rd Intersection -> LRT Ankang Sta -> Chi Jen High School -> Shisifen -> Guanghua St Entrance -> LRT Sunshine Sports Park Sta -> Shitoucuo -> Anhe Rd Sec 2 -> Yangguang Park (Xindian Flower Market) -> Yongping Village -> LRT Xinhe Elementary School Sta(Hydraulic building) -> Xinhe Elementary School -> Sanjie Temple -> Jingxin St -> Xinxin Xiangrong(Jingxin St) -> Jingxin St Intersection -> MRT Jingan Sta[^tdx-bus]

**8 (30 stops):** Zhonghe Church -> Jingxin St -> Sanjie Temple -> Xinhe Elementary School -> LRT Xinhe Elementary School Sta(Hydraulic building) -> Yongping Village -> Yangguang Park (Xindian Flower Market) -> Anhe Rd Sec 2 -> Shitoucuo -> LRT Sunshine Sports Park Sta -> Guanghua St Entrance -> Shisifen -> Chi Jen High School -> LRT Ankang Sta -> Guanghua New Village(Ankang Rd) -> Anhua Rd Intersection -> Xiacheng Community -> Mansion of General Hu-Lien -> Dingcheng -> Dapingding -> Xitou -> Bitan Bridge -> Binlang Rd -> MRT Xindian City Hall Sta(Beixin) -> Zhongzheng Rd -> Zhongxing&Zhongzheng Rd Intersection -> Wufong Junior High School -> Zhongxin Rd Sec 1 -> Old Postal Hall -> MRT Xindian Sta[^tdx-bus]
