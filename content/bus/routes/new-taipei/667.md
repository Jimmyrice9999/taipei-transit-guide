---
title: "667 / 667"
summary: "New Taipei route: 667 (667)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 667 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 31 stops from 干城路口, past 廣福公園, then 莒光路, then 後埔國小, then 中山國中(縣民大道), then 雙十路口, then 捷運西門站, ending at 中華路北站.[^tdx-bus] 3 confirmed interchanges are recorded in the curated stop-ID join set: Fuzhong, Banqiao and Ximen.[^tdx-bus] The route runs 2 subroutes under a single operator, 臺北客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 67 distinct stops in total: 31 stops (outbound); 36 stops (return).[^tdx-bus] The published fare is charged over 1 segments; the published route length is 25.6 km; weekday service runs from 05:30 to 22:30, with a published peak headway of 0710 minutes and 1015 minutes off-peak; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “板橋-臺北車站”; published distance 25.6 km; service span Weekdays: 去程：0530-2230；返程：0530-2230; Holidays: 去程：0530-2230；返程：0530-2230; headway fields Weekdays: weekday peak 0710; weekday off-peak 1015; Holidays: holiday peak 10; holiday off-peak 15; fare 一段票 / 1 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 667; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**667 (31 stops):** Gancheng Rd Entrance -> Guangfu Park -> Wuquan Park -> Shoude St Entrance -> Huaan St Entrance -> Ziqiang Elementary School -> Guoguang Police Station -> Juguang Rd -> DeGuang Rd Entrance -> Deguang Rd -> Minde Rd -> Fude Village -> Houpu Elementary School -> Shijian & Chengdu Intersectuon -> Jieshou Park -> Minquan Rd Entrance -> New Taipei City Hall(Xinfu Rd) -> NTPC Banqiao Bus Station -> Zhongshan Junior High School(Xian min Blvd) -> Huacui Bridge -> Taihe St Entrance -> Shuangyu Village -> Haishantianxia Community -> Shuangshi Rd Entrance -> Shuangyuan St Entrance -> Chian Times Building -> Wanhua Station -> Bangka Blvd -> TCUH, Heping Branch -> MRT Ximen Sta -> Zhonghua Rd North Stop[^tdx-bus]

**667 (36 stops):** Taipei Main Station (Chongqing) -> Taipei Post Office (Futai Street Mansion) -> Zhonggua Rd North Stop -> MRT Ximen Station -> Xiaonanmen -> TCUH Heping Branch -> Bangka Blvd -> Wanhua Station -> Chian Times Building -> Shuangyuan St Entrance -> Shuangshi Rd Entrance -> Fushan St Entrance -> Shuangyu Village -> Huacui Bridge -> Zhongshan Junior High School(Xian min Blvd) -> NTPC Banqiao Bus Station -> New Taipei City Hall(Xinfu Rd) -> Minquan Rd Entrance -> Banqiao Bus Terminal Extension -> Old Banqiao Sta Shopping Circle -> Jieshou Park(Guanqian E Rd) -> Shijian & Chengdu Intersectuon -> Houpu Elementary School -> Fude Village -> Minde Rd -> Deguang Rd -> DeGuang Rd Entrance -> Juguang Rd -> Guoguang Police Station -> Ziqiang Elementary School -> Huaan St Entrance -> Shoude St Entrance -> Wuquan Park -> Guangfu Park -> Gancheng Rd Entrance -> Old Banqiao Sta[^tdx-bus]
