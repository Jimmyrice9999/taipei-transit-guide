...
The committed TDX snapshot carries 1 directional or variant stop sequence for Shulin Train Station Rear-Ocean Park; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**Shulin Train Station Rear-Ocean Park (4 stops):** Shulin Train Station Rear -> Shulin Renai Hospital -> Dongsheng Fude Temple -> Ocean Park[^tdx-bus]
