---
title: "638Sub / 638副"
summary: "New Taipei route: 638Sub (638副)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 638副 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 33 stops from 五股站, past 陸光國宅一, then 五股國小, then 助順將軍廟, then 大窠橋, then 同榮里, then 輔大醫院(貴子路), ending at 輔大醫院.[^tdx-bus] 3 confirmed interchanges are recorded in the curated stop-ID join set: Taishan Guihe Station, Fu Jen University and Danfeng.[^tdx-bus] The route runs 2 subroutes under a single operator, 三重客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 73 distinct stops in total: 33 stops (outbound); 40 stops (return).[^tdx-bus] The published fare is charged over 1 sections; the published route length is 21.6 km; weekday service runs from 06:00 to 22:00; holiday service instead runs from 08:00 to 12:30.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “五股-捷運輔大站”; published distance 21.6 km; service span Weekdays: 去程：0600-2200；返程：0600-2200; Holidays: 去程：0800-1230；返程：0800-1230; fare 一段票 / 1 sections.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 638Sub; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**638 Sub (33 stops):** Wugu Sta -> Luguang Public Housing I -> Luguang Public Housing -> Wufu Community -> Community Activity Center -> Bank SinoPac -> Wugu Service Society -> Wugu Elementary School -> Penglaikeng -> Military Police Academy -> Ming Ri Shi Jie -> Ziqiang Rd Entrance -> Apartments for the Elderly -> Zhushun General Temple -> Zhulin Road -> Maoshang Public Housing -> Mingde Rd Entrance -> Deyin Elementary School -> Far Eastern Logistic Co -> Dake Bridge -> Mingzhi and Heping Intersection -> Taishan Post Office -> Xiataishanyan -> Taishan Dist Office -> Taishan High School -> Tongrong Village -> Yixue Village -> Wencheng Rd Entrance -> Mingzhi Village -> Mingzhi Elementary School -> Taishanyan -> Fu Jen Catholic University Hospital( Guizi Rd) -> Fu Jen Catholic University Hospital[^tdx-bus]

**638 Sub (40 stops):** MRT Fu Jen University Station -> Yingpankou -> Sanyo -> MRT Danfeng Station -> Danfeng 1 -> MRT Taishan Guihe Station(Mingzhi Rd) -> Xiapojiao -> Hongtai New Village -> Mingchi U of Technology -> Taishanyan -> Mingzhi Elementary School -> Mingzhi Village -> Wencheng Rd Entrance -> Yixue Village -> Tongrong Village -> Taishan Senior High School -> Taishan Dist Office -> Xiataishanyan -> Taishan Post Office -> Mingzhi and Heping Intersection -> Dake Bridge -> Far Eastern Logistic Co -> Deyin Elementary School -> Mingde Rd Entrance -> Maoshang Public Housing -> Zhulin Road -> Zhushun General Temple -> Apartments for the Elderly -> Ziqiang Rd Entrance -> Ming Ri Shi Jie -> Military Police Academy -> Penglaikeng -> Wugu Elementary School -> Wugu Service Center -> Bank SinoPac -> Activity Center -> Wufu Community -> Luguang Public Housing -> Luguang Public Housing I -> Wugu Sta[^tdx-bus]
