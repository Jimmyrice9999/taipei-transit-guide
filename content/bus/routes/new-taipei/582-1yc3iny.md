---
title: "582 / 582"
summary: "New Taipei route: 582 (582)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 582 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 3 stops from 立體停車場(五股公有市場), past 聯合醫院中興院區, ending at 臺北車站(鄭州).[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Taipei Main Station and Beimen.[^tdx-bus] The route runs 2 subroutes under a single operator, 國光客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 5 distinct stops in total: 3 stops (outbound); 2 stops (return).[^tdx-bus] The published fare is charged over 1 Segment; the published route length is 25.4 km; weekday service runs from 06:10 to 22:00; holiday service instead runs from 07:00 to 22:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “五股-臺北車站”; published distance 25.4 km; service span Weekdays: 去程：0610-2200；返程：0610-2200; Holidays: 去程：0700-2200；返程：0700-2200; fare 一段票 / 1 Segment.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 582; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**582 (3 stops):** Parking Lot(Wugu Market) -> Zhongxing Hospital -> Taipei Main Sta (Zhengzhou)[^tdx-bus]

**582 (2 stops):** Taipei Main Sta (Zhongxiao) -> Parking Lot(Wugu Market)[^tdx-bus]
