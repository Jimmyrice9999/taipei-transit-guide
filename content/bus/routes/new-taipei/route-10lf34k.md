...
The committed TDX snapshot carries 1 directional or variant stop sequence for gongliao; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**貢寮區學生專車 (18 stops):** Gongliao Junior High School -> Gongliao Elementary School -> Yanliao -> Jianshanjiao St Shiding Bridge -> Mazu Temple -> Aodi -> Aodi Fish Harbor -> Jingbeiban -> Huoyanshan -> Chuanliwu -> Jinsha Bay -> Hemei -> Beishikengkou -> Longdong Ocean Park -> Fozu Temple -> Longdonggang -> Longdong -> Longdong Bay Ocean Park[^tdx-bus]
