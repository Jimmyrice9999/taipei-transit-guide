---
title: "594 / 594"
summary: "New Taipei route: 594 (594)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 594 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 18 stops from 捷運淡水站, past 永樂巷口(中山市場), then 鄧公國小, then 淡大運動場, then 巧克力花園, then 淡江學園, then 鄧公里, returning to 捷運淡水站.[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Tamsui and Tamkang University.[^tdx-bus] The route runs 1 subroute under a single operator, 淡水客運.[^tdx-bus] The committed record carries 1 direction/variant sequence, 18 distinct stops in total: 18 stops (outbound).[^tdx-bus] The published fare is charged over 1 Segment; the published route length is 7.6 km; weekday service runs from 06:30 to 19:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “捷運淡水站-淡江大學”; published distance 7.6 km; service span Weekdays: 去程：0630-1900；返程：0630-1900; Holidays: 去程：0630-1900；返程：0630-1900; fare 一段票 / 1 Segment.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for 594; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**594 (18 stops):** MRT Tamsui Sta -> Yongle Lane Intersection(Zhongshan Market) -> Zhongxin Village Activity Center -> Yingzhuan Rd 122th Lane Entrance -> Denggong Elementary School -> Secret Garden -> Tamkang University City -> Tamkang Stadium -> Tamkang University -> Haijing Tainxia Community -> chocolate garden -> Huangdi Temple -> Songtao  campus Sta -> Tamkang campus -> Tamkang University City -> Denggong Elementary School -> Denggong Village -> MRT Tamsui Sta[^tdx-bus]
