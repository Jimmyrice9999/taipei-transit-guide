---
title: "F933-0635 / F933-0635"
summary: "New Taipei route: F933-0635 (F933-0635)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F933-0635 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 6 stops from 金山區公所, past 下六股, then 重和里, then 六股里潭子內, then 三界壇路99巷, returning to 金山區公所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 金山區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 6 distinct stops in total: 6 stops (outbound).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data] This is one of 10 separately catalogued F933 departure-time instances in the committed TDX layer (including F933-0620, F933-0710, F933-0800 and F933-1530), each pairing a distinct stop pattern to a single scheduled trip rather than appearing as timetable rows under one shared route record.[^tdx-bus]
...
The committed TDX snapshot carries 1 directional or variant stop sequence for F933-0635; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F933-0635 (6 stops):** Jinshan District Office -> Xialiugu Village -> Chonghe Village -> Liugu Village Tanzinei -> Lane 99 Sanjietan Rd -> Jinshan District Office[^tdx-bus]
