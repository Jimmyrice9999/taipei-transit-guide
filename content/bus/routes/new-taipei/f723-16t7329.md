---
title: "F723D / F723下石嘈"
summary: "New Taipei route: F723D (F723下石嘈)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F723下石嘈 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 16 stops from 坪林國中, past 坪林區公所, then 鶯子瀨, then 清雲橋頭, then 保安宮, then 保安宮牌樓, then 協德宮牌樓, returning to 坪林國中.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 坪林區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 16 distinct stops in total: 16 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 07:20 to 17:20; holiday service instead runs from 07:20 to 17:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “坪林國中-清雲農場”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0720-1720；返程：0720-1720; Holidays: 去程：0720-1700；返程：0720-1700; headway fields Weekdays: 到清雲農場後立即折返; Holidays: 到清雲農場後立即折返.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F723D; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F723D (16 stops):** Pinglin Junior High School -> Pinglin District Office -> Xiede Temple Arch Gate -> Baoan Temple Arch Gate -> Yingzilai -> Baoan Temple -> Qingyun Bridge -> Qingyun Farm -> Qingyun Bridge -> Baoan Temple -> Yingzilai -> Baoan Temple Arch Gate -> JIN HUA CHA JHUANG -> Jiuqionlin -> Xiede Temple Arch Gate -> Pinglin Junior High School[^tdx-bus]
