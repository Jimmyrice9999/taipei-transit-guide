---
title: "889 / 889"
summary: "New Taipei route: 889 (889)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 889 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 33 stops from 三峽一站, past 捷運三峽站, then 佳園學勤路口, then 育德, then 東山里, then 溪福里, then 合宜路(一), ending at 合宜路.[^tdx-bus] 3 confirmed interchanges are recorded in the curated stop-ID join set: Far Eastern Hospital, Sanxia and National Taipei University.[^tdx-bus] The route runs 2 subroutes under a single operator, 臺北客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 70 distinct stops in total: 33 stops (outbound); 37 stops (return).[^tdx-bus] The published fare is charged over Charge for 2 sections; the published route length is 33 km; weekday service runs from 06:20 to 20:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三峽-捷運亞東醫院站”; published distance 33 km; service span Weekdays: 去程：0620-2000；返程：0620-2000; Holidays: 去程： - ；返程： -; fare 二段票 / Charge for 2 sections.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 889; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**889 (33 stops):** Sanxia Stop 1 -> MRT Sanxia Sta -> Longpu -> Guniang Temple -> Sanshu Rd -> Agricultural Research and Extension Station -> Nanyuan -> Jiayuan Xuechin Rd Intersection -> Shibajian -> Ganyuan -> Beiganyuan -> Beiyuan -> Ganyuan Bridge -> Yude -> Shanjia -> Zhongzhou St Entrance -> Sanjia Elementary School -> Dongshan -> South Shulin Station -> Dongshan Village -> Dongrong St Entrance -> Datong Elementary School -> Heping St Entrance -> Shulin High School -> Duxing Rd Sec 3 -> Xifu Village -> Shalun Elementary School(Duxing Rd) -> Kunlun Village -> Shalun Village -> Oriental University of Science & Technology(Nanya S Rd) -> MRT Far Eastern Hospital Station -> Heyi Rd(1) -> Heyi Rd[^tdx-bus]

**889 (37 stops):** MRT Far Eastern Hospital Station -> Oriental University of Science & Technology(Nanya S Rd) -> Nanya Stop -> Shalun Village -> Kunlun Village -> Shalun Elementary School(Duxing Rd) -> Xifu Village -> Duxing Rd Sec 3 -> Shulin High School -> Heping St Entrance -> Datong Elementary School -> Dongrong St Entrance -> Dongshan Village -> South Shulin Station -> Dongshan -> Sanjia Elementary School -> Jixiang Temple -> Zhongzhou St Entrance -> Sanjia -> Yude -> Ganyuan Bridge -> Beiyuan -> Beiganyuan -> Ganyuan -> Shibajian -> Landscape Wanping Park(Gongcuozi) -> Jiayuan Xuechin Rd Intersection -> Nanyuan -> Agricultural Research and Extension Station -> Sanshu Rd -> Guniang Temple -> Longpu -> MRT Sanxia Sta -> Longne Park -> Nat'l Taipei U Housing -> MRT National Taipei University Sta(Fuxing Dayong Intersection) -> Sanxia Stop 1[^tdx-bus]
