---
title: "F317 / F317"
summary: "New Taipei route: F317 (F317)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F317 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 7 stops from 蘆洲區公所, past 溪墘, then 尼加拉瓜公園, then 新光醫院, then 榮總, then 振興醫院, ending at 陽明醫院.[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Sanmin Senior High School.[^tdx-bus] The route runs 2 subroutes under a single operator, 蘆洲區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F317蘆洲區公所、F317陽明醫院.[^tdx-bus] The committed record carries 2 direction/variant sequences, 15 distinct stops in total: 7 stops (outbound, F317蘆洲區公所); 8 stops (return, F317陽明醫院).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 13:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “蘆洲區公所-陽明醫院”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-1300；返程：0900-1400; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F317; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F317Luzhou District Office (7 stops):** Luzhou District Office -> XiQian -> NiJiaLaGua Park -> Xinguang Hospital -> Veterans General Hospital -> Cheng Hsin Medical Center -> Yangming Hospital[^tdx-bus]

**F317Yangming Hospital (8 stops):** Yangming Hospital -> Cheng Hsin Medical Center -> Veterans General Hospital I -> Veterans General Hospital -> Xinguang General Hospital -> Nicaragua Park -> XiQian -> Luzhou District Office[^tdx-bus]
