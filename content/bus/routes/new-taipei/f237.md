---
title: "F237 / F237"
summary: "New Taipei route: F237 (F237)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F237 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 42 stops from 出水坑, past 下寮, then 興福國小, then 下罟埔, then 光裕, then 忠孝路口B, then 林口, ending at 醒吾科技大學.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 4 subroutes under a single operator, 林口區公所.[^tdx-bus] The TDX record files 4 distinct stop-pattern names under this identity: F237出水坑、F237林口加油站、F237去繞麻埔、F237返繞麻埔.[^tdx-bus] The committed record carries 4 direction/variant sequences, 84 distinct stops in total: 41 stops (outbound, F237出水坑); 41 stops (return, F237林口加油站); 42 stops (outbound, F237去繞麻埔); 42 stops (return, F237返繞麻埔).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:20 to 16:40; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “出水坑-林口加油站”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0620-1640；返程：0710-1730; Holidays: 去程：0620-1640；返程：0710-1730.[^ntpc-bus-route-data]
