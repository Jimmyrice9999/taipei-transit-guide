---
title: "937A / 937A"
summary: "New Taipei route: 937A (937A)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 快速公車 dataset files 937A as a Rapid bus (快速公車) identity.[^ntpc-category-rapid]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 28 stops from 人見仁愛社區(民權路), past 頭湖國小, then 林口國民運動中心(文化三路), then 東湖頭, then 瑞士花園9, then 行政園區, then 蘭州國中, ending at 民族承德路口.[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Linkou Station and Yuanshan.[^tdx-bus] The route runs 2 subroutes under a single operator, 大都會客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 62 distinct stops in total: 28 stops (outbound); 34 stops (return).[^tdx-bus] The published fare is charged over 2 Segments; the published route length is 51.9 km; weekday service runs from 06:00 to 07:00, with a published peak headway of 1424 minutes.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “林口(去程不繞龜山)-圓山”; published distance 51.9 km; service span Weekdays: 去程：0600-0700；返程：0600-0700; Holidays: 去程： - ；返程： -; headway fields Weekdays: weekday peak 1424; fare 二段票 / 2 Segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 937A; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**937A (28 stops):** Renjianrenai Community -> Touhu Elementary School -> Hunan Village Activity Center -> Zonghufu Community -> Fulin & Minshen Intersection -> Fulin & Songbo Intersection -> Linkou Civil Sports Center(Wunhua 3rd) -> Xingfushi Community -> Wenhua 3rd & Zhongshan Intersection -> Chuncheng Lichi -> Shangpin Community -> Dongtouhu -> Versailles Community -> Rongyaozhixing Community -> Wensan & Xinyi Intersection -> Glory of the fature -> Swiss Garden Community 9 -> Simcity Societies -> RotaryPark(Wunhua 3rd) -> Wenhua 3rd & Zhongxiao Intersection -> FutureTownCommunity -> Administrative Park -> MRT Linkou Sta -> Taipei School for the Hearing Impaired -> Jiuquan & Chongqing Intersection -> Minzu and Chongqing Intersection -> Lanzhou Junior High School -> Minzu and Chengde Intersection[^tdx-bus]
