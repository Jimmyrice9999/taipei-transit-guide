---
title: "F835 / F835"
summary: "New Taipei route: F835 (F835)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F835 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 12 stops from 貢寮火車站, past 貢寮行政大樓, then 崙仔尾, then 鹽寮, then 砲台山, then 澳底廟口, then 澳底醫療救護中心, ending at 貢寮區漁會.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 貢寮區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 12 distinct stops in total: 12 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:45 to 16:50.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “貢寮火車站-貢寮區漁會”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0845-1650；返程：0845-1650; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F835; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F835 (12 stops):** Gongliao Rail Sta -> Gongliao Administration Building -> Gongliao Junior High School -> Lun Zai Wei -> Longmen -> Yanliao -> Paotai Mountain -> Jianshanjiao St Shiding Bridge -> Aodi Temple Entrance -> Aodi Park -> Aodi health protection Station -> Gongliao Fishermen's Assn[^tdx-bus]
