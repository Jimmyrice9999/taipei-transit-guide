---
title: "F109 / F109"
summary: "New Taipei route: F109 (F109)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F109 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 66 stops from 捷運淡水站, past 永樂巷口(中山市場), then 淡江農場, then 埤島35-2號, then 松益, then 華南銀行, returning to 捷運淡水站.[^tdx-bus] 4 confirmed interchanges are recorded in the curated stop-ID join set: Tamsui, Tamkang University, Danjin Beixin and Xinshi 1st Rd..[^tdx-bus] The route runs 6 subroutes under a single operator, 淡水區公所.[^tdx-bus] The TDX record files 6 distinct stop-pattern names under this identity: F109、F109經湖底路、F109經鄒厝崙、F109延駛中寮、F109經湖底路、鄒厝崙、延駛中寮、F109經湖底路、延駛中寮.[^tdx-bus] The committed record carries 6 direction/variant sequences, 66 distinct stops in total: 62 stops (outbound, F109); 64 stops (outbound, F109經湖底路); 63 stops (outbound, F109經鄒厝崙); 63 stops (outbound, F109延駛中寮); 66 stops (outbound, F109經湖底路、鄒厝崙、延駛中寮); 65 stops (outbound, F109經湖底路、延駛中寮).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 05:20 to 21:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “捷運淡水站-華南銀行”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0520-2100；返程：0520-2100; Holidays: 去程：0520-2100；返程：0520-2100.[^ntpc-bus-route-data]
