...
The confirmed stop sequence runs 7 stops from 坪林國中, past 水柳腳商圈, then 坪林遊客中心, then 林興居, then 鐵馬新樂園, then 蕨類步道, ending at 九芎根親水公園.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 坪林區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F722去、F722返.[^tdx-bus] The committed record carries 2 direction/variant sequences, 15 distinct stops in total: 7 stops (outbound, F722去); 8 stops (return, F722返).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 16:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “坪林國中-九芎根親水公園”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-1600；返程：0900-1700; Holidays: 去程：0800-1600；返程：0900-1700; headway fields Weekdays: 請參照簡圖時刻表 平日09:00九芎根發車不經茶業博物館; Holidays: 請參照簡圖時刻表.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F722; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F722 Go (7 stops):** Pinglin Junior High School -> Shuiliujiao Shopping Circle -> Pinglin Visitor Center -> Dunan Bridge -> Bicycle Paradise -> Fern Hiking Trail -> Jiuqionggen Riverside Park[^tdx-bus]

**F722 Back (8 stops):** Jiuqionggen Riverside Park -> Fern Hiking Trail -> Bicycle Paradise -> Production and Marketing 5 -> Xiede Temple -> Dalin Bridge -> Tea Museum -> Pinglin Junior High School[^tdx-bus]
