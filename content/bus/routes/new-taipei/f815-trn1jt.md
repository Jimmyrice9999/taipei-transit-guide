---
title: "F815 / F815"
summary: "New Taipei route: F815 (F815)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F815 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 15 stops from 雙溪火車站, past 綜合市場, then 辭職嶺, then 後寮子路口, then 壽山宮, then 保成坑口, then 坪溪頭, ending at 灣潭.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 雙溪區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F815雙溪火車站、F815灣潭.[^tdx-bus] The committed record carries 2 direction/variant sequences, 33 distinct stops in total: 15 stops (outbound, F815雙溪火車站); 18 stops (return, F815灣潭).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data]
...
The committed TDX snapshot carries 2 directional or variant stop sequences for F815; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F815Shuangx Rail Sta (15 stops):** Shuangxi Rail Sta -> General Market -> Taihe Building -> Cizhiling -> Qianzhenkeng Entrance -> Neighborhood 3 Taiping Village -> Houliaozi Entrance -> Hubaotan -> Shoushan Temple -> Taiping 8 Neighborhood(Grocery) -> Liao Jiao Keng -> Baocheng Kengkou -> Shengbao Temple -> Pingxitou -> Wantan[^tdx-bus]

**F815Wantan (18 stops):** Wantan -> Pingxitou -> Shengbao Temple -> Baocheng Kengkou -> Shoushan Temple -> Taiping 8 Neighborhood(Grocery) -> Liao Jiao Keng -> Baocheng Kengkou -> Shoushan Temple -> Hubaotan -> Houliaozi Entrance -> Neighborhood 3 Taiping Village -> Qianzhenkeng Entrance -> Cizhiling -> Taihe Building -> Shuangxi Senior High School -> General Market -> Shuangxi Rail Sta[^tdx-bus]
