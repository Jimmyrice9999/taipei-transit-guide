---
title: "F816 / F816"
summary: "New Taipei route: F816 (F816)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F816 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 19 stops from 澳底醫療救護中心, past 澳底臻信福, then 嶺頭觀日, then 水尾, then 番仔坑, then 赤皮崙, then 安樂區行政大樓, ending at 三十一號橋.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 雙溪區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F816澳底醫療救護中心、F816三十一號橋.[^tdx-bus] The committed record carries 2 direction/variant sequences, 38 distinct stops in total: 19 stops (outbound, F816澳底醫療救護中心); 19 stops (return, F816三十一號橋).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data]
...
The committed TDX snapshot carries 2 directional or variant stop sequences for F816; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F816 Aodi health protection Station (19 stops):** Aodi health protection Station -> Aodi Zhenxinfu -> Wenxiukeng(Datieliao) -> Shuangban Bridge -> Lingtou Guanri -> Neighborhood 8 Sangang -> Cian Temple -> Shuiwei -> Shuangxi Rail Sta -> Shuangxi Senior High School -> Shanglin 2 Neighborhood -> Fanzaikeng -> Ganjiao -> Mana Country -> Chipilun -> Nuannuan District Office -> Badu Elementary School -> Anle DistAdministration Building -> NO31 Bridge[^tdx-bus]

**F816 (19 stops):** NO31 Bridge -> Anle DistAdministration Building -> Badu Elementary School -> Nuannuan District Office -> Chipilun -> Mana Country -> Ganjiao -> Fanzaikeng -> Shanglin 2 Neighborhood -> Shuangxi Senior High School -> Shuangxi Rail Sta -> Shuiwei -> Cian Temple -> Neighborhood 8 Sangang -> Lingtou Guanri -> Shuangban Bridge -> Wenxiukeng(Datieliao) -> Aodi Zhenxinfu -> Aodi health protection Station[^tdx-bus]
