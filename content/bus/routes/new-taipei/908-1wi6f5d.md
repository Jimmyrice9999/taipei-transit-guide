---
title: "908Ext / 908延"
summary: "New Taipei route: 908Ext (908延)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 快速公車 dataset files 908延 as a Rapid bus (快速公車) identity.[^ntpc-category-rapid]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 18 stops from 中華路, past 安溪國小(李梅樹紀念館), then 大同路, then 礁溪里, then 捷運臺北大學站(復興大勇路口), then 台貿一村, then 南華路口, ending at 捷運景安站(景平路).[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: National Taipei University and Jingan.[^tdx-bus] The route runs 1 subroute under a single operator, 臺北客運.[^tdx-bus] The committed record carries 1 direction/variant sequence, 18 distinct stops in total: 18 stops (outbound).[^tdx-bus] The published fare is charged over 2 segments; the published route length is 21.5 km; weekday service runs from 06:30 to 07:30.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三峽-捷運景安站”; published distance 21.5 km; service span Weekdays: 去程：0630-0730；返程：0630-0730; Holidays: 去程： - ；返程： -; fare 兩段票 / 2 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for 908Ext; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**908 Sub (18 stops):** ZhongHua Rd -> Anxi Elementary School (The Li Mei-shu Memorial Gallery) -> Zhonghua & Datong Intersection -> Anxi Junior High School -> Datong Rd -> Jieshou Village -> Jieshou & Datong Intersection -> JiaoXi Village -> Heping St Entrance -> Sanxia Junior High School I -> MRT National Taipei University Sta(Fuxing Dayong Intersection) -> EnChuKong Hospital -> Zhongzheng Rd -> Taimao 1st Village -> Zhonghe National Tax Administration -> Zhonghe Dist Office (Jingping Rd) -> Yuantong&Jingping Rd Entrance -> MRT Jingan Sta(Jingping Rd)[^tdx-bus]
