---
title: "Ruifang-Nèikē / 瑞芳-內科(基客)"
summary: "New Taipei route: Ruifang-Nèikē (瑞芳-內科(基客))."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 跳蛙公車 dataset files 瑞芳-內科(基客) as a Jump-frog commuter bus (跳蛙公車) identity.[^ntpc-category-jumpfrog]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 14 stops from 瑞芳火車站, past 東和里, then 瑞興新村, then 四腳亭, then 麗寶大樓, then 瑞光港墘路口, then 治磐新村, ending at 美麗華(植福).[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Jiannan Rd..[^tdx-bus] The route runs 2 subroutes under a single operator, 基隆客運.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: 瑞芳-內科(基客)去、瑞芳-內科(基客)返.[^tdx-bus] The committed record carries 2 direction/variant sequences, 28 distinct stops in total: 14 stops (outbound, 瑞芳-內科(基客)去); 14 stops (return, 瑞芳-內科(基客)返).[^tdx-bus] The published fare is charged over 4 segment; the published route length is 64 km; weekday service runs from 06:40 to 17:20.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “瑞芳-內科”; published distance 64 km; service span Weekdays: 去程：0640-1720；返程：1830-1830; Holidays: 去程： - ；返程： -; fare 四段票 / 4 segment.[^ntpc-bus-route-data]
...
**Ruifang-Nèikē Go (14 stops):** Ruifang Rail Sta -> Donghe Village -> Jay Fish pit -> Ruixing New Village -> Daliao Rd Entrance -> Sijiaoting -> Baoxiang Jixiang -> Dongnuan New Village -> Libao Building -> Ruiguang Rd -> Ruiguang &Gangqian Roads -> Renbao Bldg -> Zhipan New Village -> Miramar (Zhifu)[^tdx-bus]

**Ruifang-Nèikē Back (14 stops):** Miramar (Zhifu) -> Zhipan New Village -> Renbao Bldg -> Ruiguang & Gangqian Intersection -> Ruiguang Rd -> Libao Building -> Dongnuan New Village -> Baoxiang Jixiang -> Sijiaoting -> Daliao -> Ruixing New Village -> Jay Fish pit -> Donghe Village -> Resident Square[^tdx-bus]
