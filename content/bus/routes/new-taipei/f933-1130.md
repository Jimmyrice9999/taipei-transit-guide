---
title: "F933-1130 / F933-1130"
summary: "New Taipei route: F933-1130 (F933-1130)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F933-1130 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 12 stops from 臺大醫院金山分院, past 金山區公所, then 金山分局, then 豐漁, then 山尾, then 磺港路, then 磺港, then 社寮, then 金山青年活動中心, then 金山分局, then 金山, ending at 金山區公所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 金山區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 12 distinct stops in total: 12 stops (outbound).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data] This is one of 10 separately catalogued F933 departure-time instances in the committed TDX layer (including F933-0620, F933-0635, F933-0710 and F933-0800), each pairing a distinct stop pattern to a single scheduled trip rather than appearing as timetable rows under one shared route record.[^tdx-bus]
...
The committed TDX snapshot carries 1 directional or variant stop sequence for F933-1130; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F933-1130 (12 stops):** Nat'l Taiwan U Hospital Jinshan Branch -> Jinshan District Office -> Jinshan Police Sta -> Fengyu -> Shanwei -> Huanggang Rd -> Huanggang -> Sheliao -> Jinshan Youth Activity Center -> Jinshan Police Sta -> JinShan -> Jinshan District Office[^tdx-bus]
