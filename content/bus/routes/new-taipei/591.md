---
title: "591 / 591"
summary: "New Taipei route: 591 (591)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 591 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 15 stops from 水蓮山莊, past 北峰寺, then 北峰里(明峰街), then 經貿園區, then 東湖國小, then 南港軟體園區北站, then 南港高工(南港路), ending at 南港車站.[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Nangang and Donghu.[^tdx-bus] The route runs 2 subroutes under two operators, 新北客運 and 中興巴士.[^tdx-bus] The committed record carries 2 direction/variant sequences, 29 distinct stops in total: 15 stops (outbound); 14 stops (return).[^tdx-bus] The published fare is charged over 1 Segment; the published route length is 11.3 km; weekday service runs from 06:30 to 21:00; holiday service instead runs from 07:00 to 20:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “水蓮山莊-南港車站”; published distance 11.3 km; service span Weekdays: 去程：0630-2100；返程：0630-2100; Holidays: 去程：0700-2000；返程：0700-2000; fare 一段票 / 1 Segment.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 591; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**591 (15 stops):** Lotus Hill -> Beifeng Temple -> Jinlong Villiage -> Beifong Village(Mingfong St) -> Jinlong Elementary School(Mingfong St) -> Munan Coal Mine -> Economic & Trade Park -> Beishan Village -> Donghu Elementary School -> Wufen Community -> MRT Donghu Sta (Nanhu High School) -> Nangang Software Park N -> Nangang Software Park S -> Nangang Vocational High School (Nangang Rd) -> Nangang Rail Sta[^tdx-bus]

**591 (14 stops):** Nangang Dist Admin Center(Nangang Rail Sta) -> Nangang Vocational High School (Nangang Rd) -> Nangang Software Park S -> Nangang Software Park N -> MRT Donghu Sta (Nanhu High School) -> Wufen Community -> Donghu Elementary School -> Beishan Village -> Economic & Trade Park -> Munan Coal Mine -> Jinlong Elementary School(Mingfong St) -> Jinlong Villiage -> Beifeng Temple -> Lotus Hill[^tdx-bus]
