---
title: "F802 / F802"
summary: "New Taipei route: F802 (F802)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F802 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 20 stops from 龍安宮, past 祖師廟, then 瑞芳公園入口, then 九份派出所, then 福山宮, then 保民堂, then 瓜山國小(戰俘營), ending at 祈堂廟.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 3 subroutes under a single operator, 瑞芳區公所.[^tdx-bus] The TDX record files 3 distinct stop-pattern names under this identity: F802龍安宮、F802祈堂廟、F802輔助線.[^tdx-bus] The committed record carries 3 direction/variant sequences, 46 distinct stops in total: 20 stops (outbound, F802龍安宮); 21 stops (return, F802祈堂廟); 6 stops (outbound, F802輔助線).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:30 to 15:30; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “瑞芳-金九線”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0830-1530；返程：0915-1630; Holidays: 去程：0830-1530；返程：0915-1630; headway fields Weekdays: 16:30班次另行至欽賢國中及浪漫公路.[^ntpc-bus-route-data]
