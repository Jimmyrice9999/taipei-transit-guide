---
title: "F721 / F721"
summary: "New Taipei route: F721 (F721)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F721 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 6 stops from 坪林國中, past 漁光國小, then 南山寺, then 漁光國小, then 英速魔法學院, ending at 闊瀨.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 4 subroutes under a single operator, 坪林區公所.[^tdx-bus] The TDX record files 4 distinct stop-pattern names under this identity: F721坪林國中至闊瀨、F721闊瀨至坪林國中、F721坪林國中至南山寺、F721南山寺至坪林國中.[^tdx-bus] The committed record carries 4 direction/variant sequences, 12 distinct stops in total: 6 stops (outbound, F721坪林國中至闊瀨); 5 stops (return, F721闊瀨至坪林國中); 3 stops (outbound, F721坪林國中至南山寺); 3 stops (return, F721南山寺至坪林國中).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; no weekday service is published; holiday service runs from 08:00 to 16:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “坪林國中-南山寺”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程： - ；返程： -; Holidays: 去程：0800-1600；返程：0900-1700.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 4 directional or variant stop sequences for F721; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F721 Pinglin Junior High School-English Wonderland (6 stops):** Pinglin Junior High School -> Yuguang Elementary School -> Nanshan Temple -> Yuguang Elementary School -> English Wonderland -> Kuolai[^tdx-bus]

**F721 English Wonderland-Pinglin Junior High School (5 stops):** Kuolai -> English Wonderland -> Yuguang Elementary School -> Nanshan Temple -> Pinglin Junior High School[^tdx-bus]

**F721 Pinglin Junior High School-Nanshan Temple (3 stops):** Pinglin Junior High School -> Yuguang Elementary School -> Nanshan Temple[^tdx-bus]

**F721 Nanshan Temple-Pinglin Junior High School (3 stops):** Nanshan Temple -> Yuguang Elementary School -> Pinglin Junior High School[^tdx-bus]
