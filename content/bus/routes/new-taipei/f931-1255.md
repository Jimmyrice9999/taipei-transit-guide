---
title: "F931-1255 / F931-1255"
summary: "New Taipei route: F931-1255 (F931-1255)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F931-1255 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 15 stops from 金山區公所, past 臺大醫院金山分院, then 磺溪中橋, then 六股中路, then 重和里, then 葵扇湖, then 兩湖里, then 兩福宮, then 半嶺(李簡玉英路口), then 三界里(7、8鄰), then 三界里(10鄰), then 三界聖德宮, then 磺溪橋, returning to 金山區公所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 金山區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 15 distinct stops in total: 15 stops (outbound).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data]
...
The committed TDX snapshot carries 1 directional or variant stop sequence for F931-1255; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F931-1255 (15 stops):** Jinshan District Office -> Nat'l Taiwan U Hospital Jinshan Branch -> Huangxizhong Bridge -> Liuguzhong Rd -> Chonghe Village -> Kuishan Lake -> Lianghu Village -> Lianghu Village(Neighborhood 3) -> Liangfu Temple -> Banling -> Sanjie Village(Neighborhood 7&8) -> Sanjiegong Village -> Sanjie Shengde Temple -> HuangXi Bridge -> Jinshan District Office[^tdx-bus]
