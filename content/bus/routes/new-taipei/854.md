---
title: "854 / 854"
summary: "New Taipei route: 854 (854)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 854 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 17 stops from 華亞園區, past 大崗國中, then 長庚醫院, then 未來城社區, then 扶輪公園(文化三路), then 未來之丘, then 幸福市, ending at 林口國民運動中心(文化三路).[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Linkou Station.[^tdx-bus] The route runs 2 subroutes under a single operator, 三重客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 34 distinct stops in total: 17 stops (outbound); 17 stops (return).[^tdx-bus] The published fare is charged over 1 segments; the published route length is 13.46 km; weekday service runs from 06:00 to 21:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “華亞園區-泰北路”; published distance 13.46 km; service span Weekdays: 去程：0600-2100；返程：0600-2100; Holidays: 去程：0600-2100；返程：0600-2100; fare 一段票 / 1 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 854; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**854 (17 stops):** Hwa Ya Technology Park -> Dagang Junior High School -> Wenhua 2nd -> Park Rd Entrance -> Chang-Gung Memorial Hospital -> Linkou Bus Station -> Administrative Park -> FutureTownCommunity -> Wenhua 3rd & Zhongxiao Intersection -> RotaryPark(Wunhua 3rd) -> Simcity Societies -> Swiss Garden Community 9 -> Glory of the fature -> Wunhua 3rd & Jixiangi Intersection -> Wensan & Xinyi Intersection -> Xingfushi Community -> Linkou Civil Sports Center(Wunhua 3rd)[^tdx-bus]

**854 (17 stops):** Taibei Rd -> Linkou Civil Sports Center -> Wunhua 2 & Zhongshan Intersection -> Xinyiguobao Community -> FutureCityCommunity -> Kongjiian Park Community -> ShimaoFanglin -> Zhongxiao Rd Entrance -> Linkou Mitsui Outlet Park -> Andun Community -> Wenhua 2nd Rd Sec 1 Post Office -> Linkou Bus Station -> Chang-Gung Memorial Hospital -> Park Rd Entrance -> Wenhua 2nd -> Dagang Junior High School -> Hwa Ya Technology Park[^tdx-bus]
