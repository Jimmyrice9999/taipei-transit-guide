---
title: "F138new / F138新古庄"
summary: "New Taipei route: F138new (F138新古庄)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F138新古庄 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 12 stops from 二號倉庫, past 新庄子路口, then 快樂別墅, then 民主水口公王廟, then 八仙宮, then 大田寮, then 車路崎, returning to 二號倉庫.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 三芝區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 12 distinct stops in total: 12 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:40 to 17:30; holiday service instead runs from 07:30 to 10:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “新庄子路口-二號倉庫”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0640-1730；返程：0640-1730; Holidays: 去程：0730-1000；返程： -; headway fields Weekdays: 平日06:40、16:20延駛至新古庄里12鄰103號前.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F138new; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F138 new (12 stops):** No2 Warehouse -> Xinzhuangzi Rd Entrance -> Brick kilns -> Happy Villa -> Sunshine Coast Villa -> Minzhu Shuikou Gongwang Temple -> Baxian Temple -> Neighborhood 12 No103 -> Datianliao -> Sanzhan Bridge -> Cheluqi -> No2 Warehouse[^tdx-bus]
