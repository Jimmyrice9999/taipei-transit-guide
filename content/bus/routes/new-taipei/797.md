---
title: "797 / 797"
summary: "New Taipei route: 797 (797)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 797 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 47 stops from 明日世界, past 德音國小, then 義學里, then 丹鳳一, then 捷運新莊站(新莊郵局), then 重新大橋, then 圓環(南京), ending at 臺北車站(承德).[^tdx-bus] 7 confirmed interchanges are recorded in the curated stop-ID join set: Taishan Guihe Station, Ximen, Beimen, Xianse Temple, Touqianzhuang, Fu Jen University and Danfeng.[^tdx-bus] The route runs 2 subroutes under a single operator, 指南客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 87 distinct stops in total: 47 stops (outbound); 40 stops (return).[^tdx-bus] The published fare is charged over 2 segments; the published route length is 34.7 km; weekday service runs from 06:10 to 17:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “五股-臺北”; published distance 34.7 km; service span Weekdays: 去程：0610-1700；返程：0610-1700; Holidays: 去程： - ；返程： -; fare 二段票 / 2 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 797; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**797 (47 stops):** Ming Ri Shi Jie -> Deyin Elementary School -> Far Eastern Logistic Co -> Dake Bridge -> Mingzhi and Heping Intersection -> Taishan Post Office -> Xiataishanyan -> Taishan Dist Office -> Taishan High School -> Tongrong Village -> Yisyue Village -> Wencheng Rd Intersection -> Mingzhi Village -> Mingchi Elementary School -> Taishanyan -> Mingchi University of Technology -> Hongtai New Village -> Xiapojiao -> MRT Taishan Guihe Station(Mingzhi Rd) -> Danfeng 1 -> MRT Danfeng Station -> Sanyo -> Yingpankou -> MRT Fu Jen University Station -> Institute for the Blind -> Haishan Village -> Xintai Rd Entrance -> MRT Xinzhuang Sta(Xinzhuang Post Office) -> Xinzhuang Elementary School -> Baoyuan Temple -> MRT Touqianzhuang Station -> Touqian -> Ginling Girls' High School -> Zhongxing St Entrance -> MRT Xianse Temple Sta -> Wuguwang Temple -> Chongxin Bridge -> Zushi Temple (GuiYang) -> Guiyang St -> MRT Ximen Sta -> Zhonghua Rd North Station -> MRT Beimen Station -> Nanjing W Rd Entrance (Tacheng) -> Yanping Stop 1 (Nanjing) -> Taipei Circle -> Taipei Circle (Nanjing) -> Taipei Main Sta (Chengde)[^tdx-bus]

**797 (40 stops):** MRT Beimen Station -> Zhonghua Rd N Stop -> Ximen Market (Chengdu) -> Ximen Elementary School -> Chongxin Bridge -> Wuguwang Temple -> MRT Xianse Temple Sta -> Zhongxing St Entrance -> Ginling Girls' High School -> Touqian -> MRT Touqianzhuang Station -> Dazhong Temple -> MRT Xinzhuang Sta(Xinzhuang Post Office) -> Xintai Rd Entrance -> Haishan Village -> Institute for the Blind -> MRT Fu Jen University Station -> Yingpankou -> Sanyo -> MRT Danfeng Station -> Danfeng 1 -> MRT Taishan Guihe Station(Mingzhi Rd) -> Xiapojiao -> Hongtai New Village -> Mingchi U of Technology -> Taishanyan -> Mingzhi Elementary School -> Mingzhi Village -> Wencheng Rd Intersection -> Yixue Village -> Tongrong Village -> Taishan Senior High School -> Taishan District Office -> Xiataishanyan -> Taishan Post Office -> Mingzhi and Heping Intersection -> Dake Bridge -> Far Eastern Logistic Co -> Deyin Elementary School -> Ming Ri Shi Jie[^tdx-bus]
