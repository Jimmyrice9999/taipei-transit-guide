---
title: "F319 / F319"
summary: "New Taipei route: F319 (F319)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F319 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 31 stops from 蘆洲區公所, past 鷺江國小(三民路), then 正和街口, then 長安街一, then 長榮光明路口, then 捷運蘆洲站, then 湧蓮寺, ending at 捷運三民高中站(復興路一).[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Sanmin Senior High School and Luzhou.[^tdx-bus] The route runs 1 subroute under a single operator, 蘆洲區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 31 distinct stops in total: 31 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 07:05 to 18:50.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “蘆洲區公所-捷運三民高中站(復興路)”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0705-1850；返程：0705-1850; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F319; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F319 (31 stops):** Luzhou District Office -> Lujiang Elementary School(Sanmin Rd) -> Luzhou Traingle Park -> Luzhou Police Station -> Luzhou Motor Vehicles Office(Zhongzheng Rd) -> No18 Zhenghe St -> No58 Zhenghe St -> Zhenghe St Entrance -> Wanquan Market -> Jiuqiong St -> Yanping Village -> Zhongyi Temple -> Changan St 1 -> Yongkang Park -> Chenggong Elementary School -> Changan St -> Lane 38 Yongle St -> Yongping Market -> Changrong Guangming Rd Enterance -> Changrong & Zhongyuan Rd -> Zhongyuan Park -> Lusenbao Community -> Liuti Park -> MRT Luzhou Sta -> Zhongzheng & Guangrong intersection -> Luzhou Bus Terminal -> Wangye Temple Entrance -> National Open Univ(Zhongzheng Rd) -> Zhongyuan Apartment -> Yonglian Temple -> MRT Sanmin Senior High School Sta(Fuxing Rd )[^tdx-bus]
