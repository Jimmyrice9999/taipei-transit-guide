---
title: "F723U / F723上石嘈"
summary: "New Taipei route: F723U (F723上石嘈)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F723上石嘈 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 14 stops from 坪林國中, past 坪林區公所, then 保安宮牌樓, then 乾溝子, then 三份子, then 碧湖橋, then 山林, ending at 關聖宮.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 坪林區公所.[^tdx-bus] The committed record carries 2 direction/variant sequences, 28 distinct stops in total: 14 stops (outbound); 14 stops (return).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 16:30; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “坪林國中-關聖宮”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-1630；返程：0800-1630; Holidays: 去程：0800-1630；返程：0800-1630.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F723U; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F723U (14 stops):** Pinglin Junior High School -> Pinglin District Office -> Xiede Temple Arch Gate -> Baoan Temple Arch Gate -> CING-JIE-DUEI -> Gangouzi -> Shicao Police Station -> Wufenzi -> Sanfenzi -> Qingshan Farm -> Bihu Bridge -> Bihu -> Shanlin -> Guansheng Temple[^tdx-bus]

**F723U (14 stops):** Guansheng Temple -> Shanlin -> Bihu -> Bihu Bridge -> Qingshan Farm -> Sanfenzi -> Shicao Police Station -> Gangouzi -> CING-JIE-DUEI -> Baoan Temple Arch Gate -> JIN HUA CHA JHUANG -> Jiuqionlin -> Xiede Temple Arch Gate -> Pinglin Junior High School[^tdx-bus]
