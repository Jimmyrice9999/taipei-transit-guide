---
title: "781 / 781"
summary: "New Taipei route: 781 (781)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 781 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 18 stops from 雙溪火車站, past 分駐所, then 白灰厝, then 番仔坑, then 彎穹, then 內柑腳, ending at 長源.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 國光客運.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: 781雙溪火車站、781長源.[^tdx-bus] The committed record carries 2 direction/variant sequences: 18 stops (outbound, 781雙溪火車站); 18 stops (return, 781長源).[^tdx-bus] The published fare is charged over 1 segments; the published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 07:45 to 19:45; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “雙溪火車站-長源”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0745-1945；返程：0530-1755; Holidays: 去程：0745-1945；返程：0530-1755; fare 一段票 / 1 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 781; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**781 Shuangx Rail Sta (18 stops):** Shuangxi Rail Sta -> Police Station -> Shuangxi High School -> Puwei -> Pinglin Village -> Baihuicuo -> Pinglin Activity Center -> Lincuoqian -> Zhuliaokengkou -> Fanzaikeng -> Shanglin -> Diaoqiaotou -> Wanqiong -> Pingshui -> Fenkeng -> Ganjiao -> Neiganjiao -> Changyuan[^tdx-bus]

**781 Changyuan (18 stops):** Changyuan -> Neiganjiao -> Ganjiao -> Fenkeng -> Pingshui -> Wanqiong -> Diaoqiaotou -> Shanglin -> Fanzaikeng -> Zhuliaokengkou -> Lincuoqian -> Pinglin Activity Center -> Baihuicuo -> Pinglin Village -> Puwei -> Shuangxi High School -> Police Station -> Shuangxi Rail Sta[^tdx-bus]
