---
title: "F822 / F822"
summary: "New Taipei route: F822 (F822)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F822 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 7 stops from 平溪, past 外店, then 田子, then 白鶯橋, then 紫來, then 自來水廠, ending at 東勢格派出所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 平溪區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F822平溪、F822東勢格派出所.[^tdx-bus] The committed record carries 2 direction/variant sequences, 14 distinct stops in total: 7 stops (outbound, F822平溪); 7 stops (return, F822東勢格派出所).[^tdx-bus] The published fare is charged over Free; the published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:50 to 16:10; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “平溪-東勢派出所”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0650-1610；返程：0710-1640; Holidays: 去程：0650-1610；返程：0710-1640; fare 免費 / Free.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F822; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F822Pingxi (7 stops):** Pingxi -> Waidian -> Tianzi -> Baiying Bridge -> Zilai -> TWD Purification Plant -> Dongshige Station[^tdx-bus]

**F822Dongshige (7 stops):** Dongshige Station -> TWD Purification Plant -> Zilai -> Baiying Bridge -> Tianzi -> Waidian -> Pingxi[^tdx-bus]
