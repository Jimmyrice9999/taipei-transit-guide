---
title: "F135 / F135"
summary: "New Taipei route: F135 (F135)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F135 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 2 stops directly from 三芝國中 to 關渡國中(關渡醫院), with no intermediate stop recorded.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 三芝區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F135三芝國中、F135關渡國中(關渡醫院).[^tdx-bus] The committed record carries 2 direction/variant sequences, 5 distinct stops in total: 2 stops (outbound, F135三芝國中); 3 stops (return, F135關渡國中(關渡醫院)).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 08:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三芝國中-關渡國中(關渡醫院)”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-0800；返程：0840-0840; Holidays: 去程：0800-0800；返程：0840-0840; headway fields Weekdays: 去程0800班次經101線 回程0840班次經台2線; Holidays: 去程0800班次經101線 回程0840班次經台2線.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F135; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F135	 Sanzhi Junior High School (2 stops):** Sanzhi Junior High School -> Guandu Junior High School (Guandu Hospital)[^tdx-bus]

**F135	Guandu Junior High School (Guandu Hospital) (3 stops):** Guandu Junior High School (Guandu Hospital) -> Mackay Memorial Hospital -> Sanzhi Junior High School[^tdx-bus]
