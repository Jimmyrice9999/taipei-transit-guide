---
title: "F813 / F813"
summary: "New Taipei route: F813 (F813)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F813 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 31 stops from 澳底臻信福, past 文秀坑(打鐵寮), then 雙溪火車站, then 弓橋里活動中心, then 瑞芳火車站, then 瑞柑新村, then 牡丹車站, ending at 雙溪火車站.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 雙溪區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F813澳底、F813雙溪火車站.[^tdx-bus] The committed record carries 2 direction/variant sequences, 31 distinct stops in total: 31 stops (outbound, F813澳底); 24 stops (outbound, F813雙溪火車站).[^tdx-bus] No weekday service is published.[^ntpc-bus-route-data]
...
The committed TDX snapshot carries 2 directional or variant stop sequences for F813; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F813 Aodi (31 stops):** Aodi Zhenxinfu -> Wenxiukeng(Datieliao) -> Shuangban Bridge -> Lingtou Guanri -> Neighborhood 8 Sangang -> Cian Temple -> Shuiwei -> Shuangxi Rail Sta -> Mudan Rail Sta -> Sandiao Shibikeng -> Liucuo -> Japan shrine -> Gongqiao Village Activity Center -> Houtong Elementary School -> Rueigansincun -> Ruifang Park -> Putou -> Resident Square -> RueiFang Station -> Zushi Temple -> Resident Square -> Putou -> Ruifang Park -> Rueigansincun -> Houtong Elementary School -> Gongqiao Village Activity Center -> Japan shrine -> Liucuo -> Sandiao Shibikeng -> Mudan Rail Sta -> Shuangxi Rail Sta[^tdx-bus]

**F813 Shuangx Rail Sta (24 stops):** Shuangxi Rail Sta -> Mudan Rail Sta -> Sandiao Shibikeng -> Liucuo -> Japan shrine -> Gongqiao Village Activity Center -> Houtong Elementary School -> Rueigansincun -> Ruifang Park -> Putou -> Resident Square -> RueiFang Station -> Zushi Temple -> Resident Square -> Putou -> Ruifang Park -> Rueigansincun -> Houtong Elementary School -> Gongqiao Village Activity Center -> Japan shrine -> Liucuo -> Sandiao Shibikeng -> Mudan Rail Sta -> Shuangxi Rail Sta[^tdx-bus]
