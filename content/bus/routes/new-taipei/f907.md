---
title: "F907 / F907"
summary: "New Taipei route: F907 (F907)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F907 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 19 stops from 汐止, past 汐止公園, then 長江街13巷口, then 鹽水洞, then 大慶原鄉社區, then 五堵火車站, then 國泰醫院, returning to 汐止.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 3 subroutes under a single operator, 汐止區公所.[^tdx-bus] The TDX record files 3 distinct stop-pattern names under this identity: F907、F907延國泰醫院(建成路)、F907延汐止公園(對面).[^tdx-bus] The committed record carries 3 direction/variant sequences, 20 distinct stops in total: 18 stops (outbound, F907); 19 stops (outbound, F907延國泰醫院(建成路)); 19 stops (outbound, F907延汐止公園(對面)).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:10 to 20:30; holiday service follows the same published span.[^ntpc-bus-route-data]
