---
title: "F931-0600 / F931-0600"
summary: "New Taipei route: F931-0600 (F931-0600)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F931-0600 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 12 stops from 金山區公所, past 重和里, then 兩湖里, then 半嶺(李簡玉英路口), then 金寶山, then 三界聖德宮, then 金山中學, returning to 金山區公所.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 金山區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 12 distinct stops in total: 12 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:00 to 06:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “金山區公所-金山區公所”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0600-0600；返程：0600-0600; Holidays: 去程： - ；返程： -; headway fields Weekdays: 三界里(10鄰)韓暑假停駛.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F931-0600; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F931-0600 (12 stops):** Jinshan District Office -> Chonghe Village -> Kuishan Lake -> Lianghu Village -> Liangfu Temple -> Banling -> Jinbaoshan -> Juming Museum -> Sanjie Shengde Temple -> HuangXi Bridge -> JinShan Senior High School -> Jinshan District Office[^tdx-bus]
