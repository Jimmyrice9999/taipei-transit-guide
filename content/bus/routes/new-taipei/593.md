---
title: "593 / 593"
summary: "New Taipei route: 593 (593)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 593 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 36 stops from 捷運淡水站, past 永樂巷口(中山市場), then 油車口, then 大庄福德廟, then 淡江豪景, then 獸醫研究所, then 淡水區農會, returning to 捷運淡水站.[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Tamsui.[^tdx-bus] The route runs 1 subroute under a single operator, 淡水客運.[^tdx-bus] The committed record carries 1 direction/variant sequence, 36 distinct stops in total: 36 stops (outbound).[^tdx-bus] The published fare is charged over 1 Segment; the published route length is 10.4 km; weekday service runs from 06:10 to 21:40; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “捷運淡水站-沙崙”; published distance 10.4 km; service span Weekdays: 去程：0610-2140；返程：0610-2140; Holidays: 去程：0610-2140；返程：0610-2140; fare 一段票 / 1 Segment.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for 593; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**593 (36 stops):** MRT Tamsui Sta -> Yongle Lane Intersection(Zhongshan Market) -> JhongJian St Entrance -> Tamsui Liberry -> Little White House (Danshui Precinct) -> Fort Santo Domingo (Aletheia U) -> VeterInary Research Institute -> Huwei Cultural and Arts Park(Martyrs' Shrine) -> YouCheKou -> Danjiang New Residentail Quarter -> TianSheng Elementary School -> Dahewenming -> Green Avenue -> Tiansheng Community -> Dazhuang Fude Temple -> Zhongzheng & Xinmin Intersection -> Meirenyu Community -> Telecommunications Materials -> Shalun Village -> Long View Hotel -> DanHai Community Activity Center -> Tamkang Haojing -> Tamsui Bus Station -> Station -> Yixin Kindergarten -> Youchekou -> Huwei Cultural and Arts Park(Martyrs' Shrine) -> VeterInary Research Institute -> Fort Santo Domingo (Aletheia U) -> Taiwan Power Company(Huwei Fishing Harbor) -> Post Office(Hobe MacKay Hospital) -> Mackay Garden(Passenger Wharf)) -> Fuyou Temple(Ferry Pier) -> Tamsui's Gold Coast Entrance -> Tamsui Farmers Association -> MRT Tamsui Sta[^tdx-bus]
