---
title: "F217 / F217"
summary: "New Taipei route: F217 (F217)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F217 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 17 stops from 下坡角, past 明志科技大學, then 文程路口, then 泰山高中, then 泰山國小, then 山腳溪橋, then 勝隆土雞城, ending at 林口長庚醫院.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 泰山區公所.[^tdx-bus] The committed record carries 2 direction/variant sequences, 33 distinct stops in total: 17 stops (outbound); 16 stops (return).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 15:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “明志路三段-林口長庚醫院”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-1500；返程：0800-1500; Holidays: 去程： - ；返程： -; headway fields Weekdays: 林口長庚醫院發車: 09:00 11:00 14:00 16:00.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F217; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F217 (17 stops):** Xiapojiao -> Mingchi U of Technology -> Taishanyan -> Mingzhi Elementary School -> Wencheng Rd Entrance -> Yixue Village -> Tongrong Village -> Taishan Senior High School -> Taishan District Office -> Taishan Elementary School -> Military Police Training Center -> Taishan Jr High School -> Shanjiao Bridge -> Yuantong Temple -> Dake Activity Center -> Shenglong -> Linkou Chang-Gung Memorial Hospital[^tdx-bus]

**F217 (16 stops):** Shenglong -> Dake Activity Center -> Yuantong Temple -> Shanjiao Bridge -> Taishan Jr High School -> Military Police Training Center -> Taishan Elementary School -> Taishan Dist Office -> Taishan High School -> Tongrong Village -> Yixue Village -> Wencheng Rd Entrance -> Mingzhi Elementary School -> Taishanyan -> Mingchi U of Technology -> Xiapojiao[^tdx-bus]
