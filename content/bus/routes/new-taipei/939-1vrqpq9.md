---
title: "939Frog Bus / 939跳蛙"
summary: "New Taipei route: 939Frog Bus (939跳蛙)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 跳蛙公車 dataset files 939跳蛙 as a Jump-frog commuter bus (跳蛙公車) identity.[^ntpc-category-jumpfrog]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 21 stops from 學成大成路口, past 大學學成路口, then 森林公園, then 敦化和平路口, then 仁愛國泰醫院, then 國父紀念館, then 松壽路口, ending at 市政府(松壽).[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 臺北客運.[^tdx-bus] The committed record carries 1 direction/variant sequence, 21 distinct stops in total: 21 stops (outbound).[^tdx-bus] The published fare is charged over 3 segments; the published route length is 30.2 km; weekday service runs from 07:00 to 07:30.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三峽-臺北市政府”; published distance 30.2 km; service span Weekdays: 去程：0700-0730；返程：0700-0730; Holidays: 去程： - ；返程： -; fare 三段票 / 3 segments.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for 939Frog Bus; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**939 (21 stops):** Xuecheng & Dacheng Rd -> Daxue and Xuecheng Rd Intersection -> Nat'l Taipei U (Main Gate) -> Xuechin Rd -> Daxue Fenglu Community(Xuechin Rd) -> Shinrin Park -> Tur ya Kar Elementary and Junior High School -> Shangsham Ruoshui Community -> Dunhua & Heping Intersection -> Xinyi & Dunhua Intersection -> Renai Junior High School(Institute of Diplomacy) -> Renai & Anhe Intersection -> Cathay General Hospital -> Renai & Yanji Intersection -> Renai & Guangfu Intersection -> Dr Sun Yat-sen Memorial Hall -> Taipei City Hall -> Taipei City Hall (Songgao Rd) -> Xingya Junior High School -> Songshou Rd Entrance -> Taipei City Hall (SongShou)[^tdx-bus]
