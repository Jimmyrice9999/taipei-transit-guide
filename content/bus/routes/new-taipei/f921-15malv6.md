---
title: "F921Holiday / F921假日"
summary: "New Taipei route: F921Holiday (F921假日)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F921假日 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 15 stops from 萬里區公所, past 萬里北基路口, then 圳頭, then 加投里, then 獅頭山公園(老街溫泉區), then 金山, then 中正路, ending at 金山醫院.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 6 subroutes under a single operator, 萬里區公所.[^tdx-bus] The TDX record files 6 distinct stop-pattern names under this identity: F921假日去-繞圳頭至獅頭山、F921假日去、F921假日去-繞圳頭至金山、F921假日返、F921假日返-獅頭山繞圳頭、F921假日返-金山繞圳頭.[^tdx-bus] The committed record carries 6 direction/variant sequences, 30 distinct stops in total: 8 stops (outbound, F921假日去-繞圳頭至獅頭山); 12 stops (outbound, F921假日去); 15 stops (outbound, F921假日去-繞圳頭至金山); 12 stops (return, F921假日返); 9 stops (return, F921假日返-獅頭山繞圳頭); 15 stops (return, F921假日返-金山繞圳頭).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; no weekday service is published; holiday service runs from 06:50 to 16:10.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “萬里區公所-金山醫院”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程： - ；返程： -; Holidays: 去程：0650-1610；返程：0650-1610; headway fields Weekdays: 請參見F921正線; Holidays: 繞駛圳頭06:50(至中山堂) 16:10   往金山區公所方向：   中山堂發車繞圳頭07:40；繞圳頭17:10.[^ntpc-bus-route-data]
