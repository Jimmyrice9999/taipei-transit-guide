---
title: "F302 / F302"
summary: "New Taipei route: F302 (F302)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F302 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 20 stops from 三重區公所(新北大道), past 聯合醫院(三重院區), then 長元西街口, then 德林寺(龍門路), then 仁愛街125巷口, then 五常公園, then 仁政街113號, ending at 仁愛街315巷口.[^tdx-bus] 3 confirmed interchanges are recorded in the curated stop-ID join set: Taipei Bridge, Cailiao and Sanchong Elementary School.[^tdx-bus] The route runs 2 subroutes under a single operator, 三重區公所.[^tdx-bus] The committed record carries 2 direction/variant sequences, 35 distinct stops in total: 20 stops (outbound); 15 stops (return).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:30 to 16:30.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三重區公所-五常郵局”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0630-1630；返程：0630-1630; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F302; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F302 (20 stops):** Sanchong City Office(New Taipei Blvd) -> New Taipei City Hospital(Sanchong Branch) -> Datong Rd Entrance -> Zhengyi&Chongxin Intersection -> Changyuan WSt Entrance -> Changshou W St Intersection -> Sanan Village -> Longmen Rd Entrance -> Delin Temple(Longmen Rd) -> Chongde Community -> Renai St Entrance -> Renai St 125th Lane Entrance -> No 97 Lane 125 Renai St -> Renyi St -> Wuchang Post Office -> Wuchang Park -> No266 Renyi St(Jushan Temple) -> No49 Renzheng St (Shengan Temple) -> No113 Renzheng St -> Renzheng St 315th Lane Entrance[^tdx-bus]

**F302 (15 stops):** Wuchang Post Office -> Renyi St -> No12 Renyi St -> Sande Activity Center -> Longbin Activity Center -> Longbin Park -> Changle St Entrance -> Changtai Police Station -> Changsheng St Intersection -> Sanchong Police Station(Fude N Rd) -> MRT Taipei Bridge Sta -> Tiantai Square -> Datong Rd Entrance -> New Taipei City Hospital(Sanchong Branch) -> Sanchong City Office(New Taipei Blvd)[^tdx-bus]
