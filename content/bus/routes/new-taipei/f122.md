---
title: "F122 / F122"
summary: "New Taipei route: F122 (F122)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F122 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence is a there-and-back working of 7 stops from 八里國小, past 八里服務站, then 八里區農會, then 舊城里, then 八里, then 八里國中, returning to 八里國小.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 1 subroute under a single operator, 八里區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 7 distinct stops in total: 7 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 12:05 to 15:55; holiday service instead runs from 07:05 to 15:55.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “八里國小-荖阡坑”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：1205-1555；返程：1205-1555; Holidays: 去程：0705-1555；返程：0705-0705.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F122; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F122 (7 stops):** Bali Elementary School -> Bali Service Sta -> Bali District Farmers' Association -> JiouCheng Village -> Bali -> Bali Junior High School -> Bali Elementary School[^tdx-bus]
