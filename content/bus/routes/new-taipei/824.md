---
title: "824 / 824"
summary: "New Taipei route: 824 (824)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 一般公車 dataset files 824 as a General bus (一般公車) identity.[^ntpc-category-general]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 15 stops from 歡仔園, past 僑中一街, then 大觀國小, then 合安一路, then 合宜路, then 板橋夜市(縣民大道), then 捷運府中站, ending at 萬坪公園.[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Fuzhong and Banqiao.[^tdx-bus] The route runs 2 subroutes under a single operator, 臺北客運.[^tdx-bus] The committed record carries 2 direction/variant sequences, 29 distinct stops in total: 15 stops (outbound); 14 stops (return).[^tdx-bus] The published fare is charged over 1 segment; the published route length is 12.3 km; weekday service runs from 06:00 to 22:00; holiday service instead runs from 07:00 to 22:00.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “歡仔園-新北板橋公車站”; published distance 12.3 km; service span Weekdays: 去程：0600-2200；返程：0600-2200; Holidays: 去程：0700-2200；返程：0700-2200; fare 一段票 / 1 segment.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for 824; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**824 (15 stops):** Huanziyuan -> Qiaozhong 1st St -> Daguan Rd Lane 28 -> Daguan Elementary School -> National Taiwan University of Arts -> Zhongshan Experimental Elementary School(Hean Rd) -> Hean 1st Rd -> Hean 1st Rd(1) -> Heyi Rd -> Heyi Rd(1) -> Kwang-Hwa Vocational High School -> Banqiao Night Market (Xianmin Blvd) -> Guanqian W Rd Entrance -> MRT Fuzhong Station -> Wanping Park[^tdx-bus]

**824 (14 stops):** NTPC Banqiao Bus Station -> MRT Fuzhong Sta -> Guanqian W Rd Entrance -> Banqiao Night Market (Xianmin Blvd) -> Kwang-Hwa Vocational High School -> Heyi Rd(1) -> Heyi Rd -> Hean 1st Rd(1) -> Hean 1st Rd -> Zhongshan Experimental Elementary School -> National Taiwan University of Arts -> Daguan Elementary School -> Daguan Rd Lane 28 -> Qiaozhong 1st St[^tdx-bus]
