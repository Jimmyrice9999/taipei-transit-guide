...
The committed TDX snapshot carries 1 directional or variant stop sequence for 853Frogbus; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**853 Frogbus (6 stops):** Sanzhi -> Linzi -> St John‘s University -> Linzi -> Beixin Rd Entrance -> MRT Hongshulin Sta[^tdx-bus]
