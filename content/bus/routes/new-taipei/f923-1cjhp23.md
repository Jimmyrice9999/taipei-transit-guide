---
title: "F923Holiday / F923假日"
summary: "New Taipei route: F923Holiday (F923假日)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F923假日 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 20 stops from 萬里區公所, past 萬里公有零售市場, then 東澳, then 頂寮, then 加投里, then 獅頭山公園(老街溫泉區), then 中正路, ending at 金山醫院.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 4 subroutes under a single operator, 萬里區公所.[^tdx-bus] The TDX record files 4 distinct stop-pattern names under this identity: F923假日去、F923假日去至野柳、F923假日返、F923假日返野柳發車.[^tdx-bus] The committed record carries 4 direction/variant sequences, 40 distinct stops in total: 20 stops (outbound, F923假日去); 6 stops (outbound, F923假日去至野柳); 20 stops (return, F923假日返); 6 stops (return, F923假日返野柳發車).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; no weekday service is published; holiday service runs from 06:20 to 16:10.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “萬里區公所-金山醫院”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程： - ；返程： -; Holidays: 去程：0620-1610；返程：0620-1610; headway fields Weekdays: 請參見F923正線; Holidays: 06:20行駛至野柳；往金山醫院方向野柳發車06:40.[^ntpc-bus-route-data]
