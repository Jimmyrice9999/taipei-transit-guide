---
title: "F806 / F806"
summary: "New Taipei route: F806 (F806)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F806 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 19 stops from 龍安宮, past 祖師廟, then 瑞芳區公所, then 深澳發電廠, then 建基, then 濱二路10號, ending at 海濱.[^tdx-bus] The curated stop-ID join set records no confirmed MRT interchange on this route.[^tdx-bus] The route runs 2 subroutes under a single operator, 瑞芳區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F806龍安宮、F806海濱.[^tdx-bus] The committed record carries 2 direction/variant sequences, 27 distinct stops in total: 19 stops (outbound, F806龍安宮); 8 stops (return, F806海濱).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:50 to 15:20.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “瑞芳-大福煤礦”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0850-1520；返程：0930-1550; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F806; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F806 Longan Temple (19 stops):** Longan Temple -> Zushi Temple -> Resident Square -> First Market -> Ruifang District Office -> Jianyifating 1 -> Ruibin -> Shen'ao Power Plant -> Shenao Fishing Port -> Ruifang Fishing Association -> Shen'ao Fishing Port(Capybara) -> Jianji -> Rongxing -> Jianji -> Shen'ao Power Plant -> Ruibin -> Bangbang -> No10 Biner Rd -> Haibin[^tdx-bus]

**F806Haibin (8 stops):** Haibin -> Bangbang -> Jianyifating 1 -> Ruifang District Office -> First Market -> Ruifang Rail Sta -> Zushi Temple -> Longan Temple[^tdx-bus]
