---
title: "F312 / F312"
summary: "New Taipei route: F312 (F312)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F312 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 32 stops from 捷運三民高中站(復興路一), past 湧蓮寺, then 灰瑤, then 仁愛廣場, then 仁愛街口, then 復興路, then 鷺江國小(民族路), ending at 蘆洲區公所.[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Sanmin Senior High School and Luzhou.[^tdx-bus] The route runs 1 subroute under a single operator, 蘆洲區公所.[^tdx-bus] The committed record carries 1 direction/variant sequence, 32 distinct stops in total: 32 stops (outbound).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:05 to 18:15.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “捷運三民高中站(復興路一)-蘆洲區公所”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0605-1815；返程：0605-1815; Holidays: 去程： - ；返程： -.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 1 directional or variant stop sequence for F312; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F312 (32 stops):** MRT Sanmin Senior High School Sta(Fuxing Rd ) -> Yonglian Temple -> Zhongyuan Apartment -> National Open Univ(Zhongzheng Rd) -> Wangye Temple Entrance -> Luzhou Bus Terminal -> MRT Luzhou Sta -> Huiyao -> No487 Zhongzheng Rd -> No455 Zhongzheng Rd -> No435 Zhongzheng Rd -> MRT Luzhou Sta(Zhongzheng Rd) -> Renai Elementary School -> Renai Square -> Renai Elementary School 1 -> Lane 101 Renai St -> Lane 93 Renai St -> Lane 50 Renai St -> Renai St Entrance -> Minyi St Entrance -> No73 Minyi St -> No85 Minquan Rd -> No132 Minquan Rd -> Minquan Rd Entrance -> Fuxing Rd -> Hutian Temple -> Zhongxing St(Fuxing Rd) -> Zhongxing St(Minzu Rd) -> Yuqing Temple -> Daqing Community -> Lujiang Elementary School(Minzu Rd) -> Luzhou District Office[^tdx-bus]
