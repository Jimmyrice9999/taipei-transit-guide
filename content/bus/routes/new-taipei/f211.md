---
title: "F211 / F211"
summary: "New Taipei route: F211 (F211)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F211 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 22 stops from 泰山公有市場, past 義學里, then 泰山巖, then 捷運泰山貴和站(貴陽街), then 宏泰新村, then 明志里, then 泰山高中, ending at 泰山公有市場一.[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Taishan Guihe Station.[^tdx-bus] The route runs 2 subroutes under a single operator, 泰山區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F211、F211第一班.[^tdx-bus] The committed record carries 2 direction/variant sequences, 22 distinct stops in total: 22 stops (outbound, F211); 12 stops (outbound, F211第一班).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 06:20 to 17:10; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “泰山公有市場-貴和社區”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0620-1710；返程：0620-1710; Holidays: 去程：0620-1710；返程：0620-1710.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F211; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F211 (22 stops):** Taishan Market -> Yixue Village -> Wencheng Rd Entrance -> Mingzhi Village -> Mingzhi Elementary School -> Taishanyan -> Mingchi U of Technology -> Hongtai New Village -> Xiapojiao -> MRT Taishan Guihe Station -> MRT Taishan Guihe Station(Mingzhi Rd) -> Xiapojiao -> Hongtai New Village -> Mingchi U of Technology -> Taishanyan -> Mingzhi Elementary School -> Mingzhi Village -> Wencheng Rd Entrance -> Yixue Village -> Tongrong Village -> Taishan Senior High School -> Taishan Public Market 1[^tdx-bus]

**F211first (12 stops):** MRT Taishan Guihe Station(Mingzhi Rd) -> Xiapojiao -> Hongtai New Village -> Mingchi U of Technology -> Taishanyan -> Mingzhi Elementary School -> Mingzhi Village -> Wencheng Rd Entrance -> Yixue Village -> Tongrong Village -> Taishan Senior High School -> Taishan Public Market 1[^tdx-bus]
