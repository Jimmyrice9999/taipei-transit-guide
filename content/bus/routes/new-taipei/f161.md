---
title: "F161 / F161"
summary: "New Taipei route: F161 (F161)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F161 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 13 stops from 草里, past 乾華, then 石門, then 老梅, then 富基漁港, then 頂新莊, then 馬偕醫院, ending at 關渡國中(關渡醫院).[^tdx-bus] 2 confirmed interchanges are recorded in the curated stop-ID join set: Hongshulin and Tamsui.[^tdx-bus] The route runs 2 subroutes under a single operator, 石門區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F161草里車站、F161關渡國中(關渡醫院).[^tdx-bus] The committed record carries 2 direction/variant sequences, 27 distinct stops in total: 13 stops (outbound, F161草里車站); 14 stops (return, F161關渡國中(關渡醫院)).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 08:00 to 08:00; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “草里車站-關渡國中(關渡醫院)”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：0800-0800；返程：1200-1200; Holidays: 去程：0800-0800；返程：1200-1200.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F161; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F161 Caoli Station (13 stops):** Caoli -> Qianhua -> Jianzilu -> Shimen -> Bengshankou -> Laomei Station -> Fugui Cape Lighthouse -> Fuji Fishing Harbor -> Demao -> Dingxinzhuang -> MRT Hongshulin Sta -> Mackay Memorial Hospital -> Guandu Junior High School (Guandu Hospital)[^tdx-bus]

**F161 Guandu Junior High School (Guandu Hospital) (14 stops):** Guandu Junior High School (Guandu Hospital) -> 馬偕醫院 -> MRT Tamsui Sta -> Sanzhi -> Dingxinzhuang -> Demao -> Fuji Fishing Harbor -> Fugui Cape Lighthouse -> Laomei Station -> Bengshankou -> Shimen -> Jianzilu -> Qianhua -> Caoli[^tdx-bus]
