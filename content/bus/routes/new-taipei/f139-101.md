---
title: "F139 / F139行經101線"
summary: "New Taipei route: F139 (F139行經101線)."
updated: 2026-08-19
facts:
  - label: Service class
...
New Taipei City Department of Transportation's full 新巴士 dataset files F139行經101線 as a New Bus community service (新巴士) identity.[^ntpc-category-newbus]
The normalized TDX record keeps this route under the New Taipei municipality group and supplies its route identity, current operator records, direction/variant records, stops and geometry.[^tdx-bus]

The confirmed stop sequence runs 2 stops directly from 三芝國中 to 捷運淡水站, with no intermediate stop recorded.[^tdx-bus] 1 confirmed interchange is recorded in the curated stop-ID join set: Tamsui.[^tdx-bus] The route runs 2 subroutes under a single operator, 三芝區公所.[^tdx-bus] The TDX record files 2 distinct stop-pattern names under this identity: F139行經101線三芝國中、F139行經101線捷運淡水站.[^tdx-bus] The committed record carries 2 direction/variant sequences, 9 distinct stops in total: 2 stops (outbound, F139行經101線三芝國中); 7 stops (return, F139行經101線捷運淡水站).[^tdx-bus] The published route length is recorded as 99 km, a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset and treated here as an unpopulated placeholder rather than a measured distance for this specific route; weekday service runs from 19:55 to 19:55; holiday service follows the same published span.[^ntpc-bus-route-data]

## Current service data

The full New Taipei route-service row publishes endpoints “三芝國中-捷運淡水站”; published distance 99 km (a value shared identically across 175 otherwise unrelated New Taipei routes in this dataset, which reads as an unpopulated placeholder rather than a measured distance for this specific route); service span Weekdays: 去程：1955-1955；返程：2215-2215; Holidays: 去程：1955-1955；返程：2215-2215.[^ntpc-bus-route-data]

## Committed stop sequence

The committed TDX snapshot carries 2 directional or variant stop sequences for F139; each sequence below is ordered from its first boarding stop to its last; this is the current data record, not an inferred route history[^tdx-bus].

**F139 Sanzhi Junior High School (2 stops):** Sanzhi Junior High School -> MRT Tamsui Station[^tdx-bus]

**F139 MRT Tamsui Sta (7 stops):** MRT Tamsui Station -> Dinghao -> Zhengde Junior High School -> TPC Dormitory -> Beitouzi -> Beixinzhuang -> Sanzhi Junior High School[^tdx-bus]
