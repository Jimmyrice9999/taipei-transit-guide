...
Changhua (彰化) is station ID 3360 in the TDX Station response retrieved on 2026-08-24.[^tdx-tra-station] That record supplies the English and Traditional Chinese names, the address, administrative city and district, station-class field, coordinates and source update time.[^tdx-tra-station] The page reports those fields as fields: it does not turn the class code 1 into a passenger-service ranking because the checked station response does not define that code in the record itself.[^tdx-tra-station]
