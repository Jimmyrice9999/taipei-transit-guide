---
title: "Ruifang — Taiwan Railways"
summary: "Ruifang (瑞芳), TDX station 7360, in 新北市; current line membership and coordinates from the 2026-08-24 snapshot."
order: 4
operator: TRA
updated: 2026-08-24
facts:
  ...
  - label: "Line memberships"
    value: "Eastern Main Line (EL); Shenao Line (SA)"
    source: tdx-tra-line
specs:
  - label: "Latitude"
    value: "25.10872"
...
Ruifang (瑞芳) is station ID 7360 in the TDX Station response retrieved on 2026-08-24.[^tdx-tra-station] That record supplies the English and Traditional Chinese names, the address, administrative city and district, station-class field, coordinates and source update time.[^tdx-tra-station] The page reports those fields as fields: it does not turn the class code 1 into a passenger-service ranking because the checked station response does not define that code in the record itself.[^tdx-tra-station]

The address field is 224001新北市瑞芳區龍潭里明燈路三段 82 號.[^tdx-tra-station] This is the source station address, not a claim about the size of the station site, the walking catchment or every entrance.[^tdx-tra-station] A street address and a coordinate answer different questions, so both are retained when present.[^tdx-tra-station]

## Position on Eastern Main Line

On the Eastern Main Line StationOfLine record, this station has sequence 4 and a supplied travelled distance of 8.9 km.[^tdx-tra-line] The preceding member is Sijiaoting (7380) and the following member is Houtong (7350).[^tdx-tra-line] These are adjacency statements about the ordered dataset; they are not a claim about the stopping pattern of a particular train.[^tdx-tra-line]

The current line membership list for this station is Eastern Main Line (EL), Shenao Line (SA).[^tdx-tra-line] A shared station can therefore appear in more than one TDX line family without the data proving that all services, rolling stock or timetables use every listed relationship.[^tdx-tra-line] Route service patterns and first or last trains are TBC on this page because the committed endpoint supplies station membership rather than a complete timetable.[^tdx-tra-line]

## Location and architecture boundary

TDX places the station at latitude 25.10872 and longitude 121.80599.[^tdx-tra-station] The guide keeps those values at the precision returned by the source and does not infer platform orientation, elevation, track count or station depth from a point coordinate.[^tdx-tra-station] Platform arrangement, lifts, tactile paths, accessible toilets, nursing rooms, platform gap, exits, staffing and station opening hours are TBC until the operator station-level material is transcribed.[^tra-history]
