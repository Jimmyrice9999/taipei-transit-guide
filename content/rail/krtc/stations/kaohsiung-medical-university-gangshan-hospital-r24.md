---
title: "Kaohsiung Medical University Gangshan Hospital (R24) — Kaohsiung Metro"
summary: "Kaohsiung Medical University Gangshan Hospital (岡山高醫), TDX station R24, with current KRTC line membership, coordinates and source update fields."
order: 24
operator: KRTC
updated: 2026-08-24
facts:
...
Kaohsiung Medical University Gangshan Hospital (岡山高醫) is station ID R24 in the KRTC TDX Station response retrieved on 2026-08-24.[^tdx-krtc-station] The record supplies the operator station UID, English and Traditional Chinese names, address, city, district, coordinates, station-class field and source update time.[^tdx-krtc-station] This page reports those fields as the source publishes them and does not convert the station-class code into an unsupported passenger-service ranking.[^tdx-krtc-station]

The TDX address is 高雄市岡山區大遼里9鄰中山南路2號, in 高雄市’s 岡山區.[^tdx-krtc-station] The coordinate returned by the same record is 22.780642, 120.30188.[^tdx-krtc-station] An address and a coordinate describe different reference fields, so neither is substituted for the other.[^tdx-krtc-station] Platform orientation, station depth, entrances, track count and surrounding walking catchment are TBC on this page because those details are not fields in the committed Station record.[^tdx-krtc-station][^krtc-station-guide-en]

## Position in the metro network

The current StationOfRoute data places this station on Red Line (R).[^tdx-krtc-station-of-route] In the Red Line direction-zero sequence it is position 24, with a cumulative distance of 28.81 km.[^tdx-krtc-station-of-route] The preceding member is Ciaotou Station (R23), and the following member is Gangshan Station (RK1).[^tdx-krtc-station-of-route] These are relationships in the published ordered dataset, not a claim that every train stops in exactly the same pattern at all times.[^tdx-krtc-station-of-route][^tdx-krtc-first-last]
