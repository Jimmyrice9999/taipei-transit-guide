---
title: Nangang
summary: Nangang (南港) — TDX station NAK, Nangang District, with tra nangang station, taipei metro and city buses named by the operator.
order: 1
operator: THSR
updated: 2026-08-24
facts:
  ...
  - label: "Traditional Chinese"
    ...
    source: tdx-thsr
  - label: "Location"
    value: "Nangang District, Taipei City"
    source: tdx-thsr
  - label: "Address"
    value: "台北市南港區南港路一段313號"
    source: tdx-thsr
  - label: "Opening date"
    value: "1 July 2016"
    source: rb-thsr
  - label: "Named transfers"
    value: "TRA Nangang Station, Taipei Metro and city buses"
...
sources:
  ...
  - id: thsrc-station
    ...
    titleOriginal: "高鐵南港站資訊"
    publisher: "Taiwan High Speed Rail Corporation (台灣高速鐵路股份有限公司)"
    url: "https://www.thsrc.com.tw/ArticleContent/2f940836-cedc-41ef-8e28-c2336ac4d2cd"
    accessed: 2026-08-24
    kind: primary
    lang: zh-Hant
...
Nangang (南港) is station ID 0990 and TDX station code NAK in the committed High-Speed Rail snapshot.[^tdx-thsr] It is the northern end of the TDX-ordered list.[^tdx-thsr] The record places it in Nangang District, Taipei City, at 台北市南港區南港路一段313號, with a position of 25.05370, 121.60709.[^tdx-thsr] Those coordinates and the postal address are separate fields in the data pull, so the page does not turn a point into a claim about the station’s whole catchment.[^tdx-thsr]

## Opening stage and system role

The Railway Bureau dates Nangang’s opening to 1 July 2016 and places it in the final station added to the original 12-station network.[^rb-thsr] The same source records the route’s 350 km length and twelve-station system, which is why this page distinguishes Nangang’s local opening date from the railway’s initial service date.[^rb-thsr] The TDX record gives the current identity and location; it does not supply an architectural history or a reason for the name, so those questions remain outside the page’s asserted facts.[^tdx-thsr]

The station’s position also matters for reading later additions. The Railway Bureau lists the 2015 additions as Miaoli, Changhua and Yunlin, followed by Nangang in 2016.[^rb-thsr] Nangang is therefore described here as part of the final station added to the original 12-station network, rather than being silently grouped with the stations that began in January 2007.[^rb-thsr]

## Connections and passenger movement

The operator’s full station page gives the address as 台北市南港區南港路一段313號 and identifies the passenger connection set as TRA Nangang Station, Taipei Metro and city buses.[^thsrc-station] The practical transfer description is deliberately narrower than a regional transport claim: it records the modes named by THSRC and does not infer an interchange from geographic proximity.[^thsrc-station] For Nangang, the useful station-to-station question is therefore whether a traveller can follow the operator’s named connection, not whether every nearby bus stop or railway platform belongs to the HSR station.[^thsrc-station]
...
THSRC’s floor navigation for Nangang lists B1 and B2.[^thsrc-station] The operator’s floor navigation is two-level, B1 and B2, reflecting the underground interchange setting rather than an elevated suburban station..[^thsrc-station] This is a modest but checkable architectural description: it tells a reader how the operator exposes the passenger levels and what transport connections the station page names, without attributing an architect or an unverified design concept.[^thsrc-station] The national Railway Bureau describes the route as approximately 73% bridges and viaducts, 18% tunnels and 9% embankment or cut sections, but that system-wide ratio does not establish Nangang’s structure on its own.[^rb-thsr]
