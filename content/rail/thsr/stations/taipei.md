---
title: Taipei
summary: Taipei (台北) — TDX station TPE, Zhongzheng District, with tra taipei station, taipei metro, airport mrt and city buses named by the operator.
order: 11
operator: THSR
updated: 2026-08-24
facts:
  ...
  - label: "Traditional Chinese"
    ...
    source: tdx-thsr
  - label: "Location"
    value: "Zhongzheng District, Taipei City"
    source: tdx-thsr
  - label: "Address"
    value: "台北市北平西路3號"
    source: tdx-thsr
  ...
  - label: "Named transfers"
    value: "TRA Taipei Station, Taipei Metro, Airport MRT and city buses"
...
sources:
  ...
  - id: thsrc-station
    title: "Taipei station information"
    titleOriginal: "高鐵台北站資訊"
    publisher: "Taiwan High Speed Rail Corporation (台灣高速鐵路股份有限公司)"
    url: "https://www.thsrc.com.tw/ArticleContent/977abb69-413a-4ccf-a109-0272c24fd490"
    accessed: 2026-08-24
    kind: primary
    lang: zh-Hant
...
Taipei (台北) is station ID 1000 and TDX station code TPE in the committed High-Speed Rail snapshot.[^tdx-thsr] It is the capital-city station immediately south of Nangang in the TDX order.[^tdx-thsr] The record places it in Zhongzheng District, Taipei City, at 台北市北平西路3號, with a position of 25.04767, 121.51698.[^tdx-thsr] Those coordinates and the postal address are separate fields in the data pull, so the page does not turn a point into a claim about the station’s whole catchment.[^tdx-thsr]

## Opening stage and system role

The Railway Bureau dates Taipei’s opening to 2 March 2007 and places it in the second opening stage, after the initial seven stations.[^rb-thsr] The same source records the route’s 350 km length and twelve-station system, which is why this page distinguishes Taipei’s local opening date from the railway’s initial service date.[^rb-thsr] The TDX record gives the current identity and location; it does not supply an architectural history or a reason for the name, so those questions remain outside the page’s asserted facts.[^tdx-thsr]

The station’s position also matters for reading later additions. The Railway Bureau lists the 2015 additions as Miaoli, Changhua and Yunlin, followed by Nangang in 2016.[^rb-thsr] Taipei is therefore described here as part of the second opening stage, after the initial seven stations, rather than being silently grouped with the stations that began in January 2007.[^rb-thsr]

## Connections and passenger movement

The operator’s full station page gives the address as 台北市北平西路3號 and identifies the passenger connection set as TRA Taipei Station, Taipei Metro, Airport MRT and city buses.[^thsrc-station] The practical transfer description is deliberately narrower than a regional transport claim: it records the modes named by THSRC and does not infer an interchange from geographic proximity.[^thsrc-station] For Taipei, the useful station-to-station question is therefore whether a traveller can follow the operator’s named connection, not whether every nearby bus stop or railway platform belongs to the HSR station.[^thsrc-station]
...
THSRC’s floor navigation for Taipei lists B1.[^thsrc-station] The operator’s page presents a B1 floor guide and a corridor-based transfer to the other rail systems; the page does not turn that passenger map into an unsupported architect attribution..[^thsrc-station] This is a modest but checkable architectural description: it tells a reader how the operator exposes the passenger levels and what transport connections the station page names, without attributing an architect or an unverified design concept.[^thsrc-station] The national Railway Bureau describes the route as approximately 73% bridges and viaducts, 18% tunnels and 9% embankment or cut sections, but that system-wide ratio does not establish Taipei’s structure on its own.[^rb-thsr]
