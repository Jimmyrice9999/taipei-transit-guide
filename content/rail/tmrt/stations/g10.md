---
title: G10 Shui-an Temple
summary: G10 Shui-an Temple (水安宮) — stop 10 of 18 on Taichung Metro's Green Line, in 南屯區.
order: 10
line: G
operator: TMRT
updated: 2026-08-23
...
Shui-an Temple is stop 10 of 18 on the Green Line, and the committed TDX record pairs the code G10 with the Traditional Chinese name 水安宮.[^tdx-tmrt] TDX places it in 南屯區 at 臺中市南屯區文心路一段519號, with a published route chainage of 9.35 km in the selected direction.[^tdx-tmrt] The address is the source-backed siting description used here; no extra catchment boundary is inferred from it.
...
The operator describes the Green Line as a steel-wheel, medium-capacity, fully driverless system using two-car trains.[^tmrt-faq] It identifies G10 as an elevated station; only G0 and G17 are ground level.[^tmrt-faq] That system-level engineering fact is kept separate from the station address because TDX publishes the latter but not the former.[^tdx-tmrt][^tmrt-faq]



The operator's current public-art roster names works at selected Green Line stations, but it does not assign a named work to G10 in the roster consulted for this page.[^tmrt-art] Whether an unlisted or later work exists here is therefore TBC; the page does not turn a list boundary into a claim that no artwork exists.
...
The committed TDX station record places G10 at sequence 10 of the Green Line, in 南屯區, with a published chainage of 9.35 km for the selected direction.[^tdx-tmrt] This is a position in the operator's station order, not a travel-time or distance-between-stations claim; TDX publishes the chainage field but not a timetable for this page.[^tdx-tmrt]
