---
title: G11 Nantun
summary: G11 Nantun (南屯) — stop 12 of 18 on Taichung Metro's Green Line, in 南屯區.
order: 12
line: G
operator: TMRT
updated: 2026-08-23
...
Nantun is stop 12 of 18 on the Green Line, and the committed TDX record pairs the code G11 with the Traditional Chinese name 南屯.[^tdx-tmrt] TDX places it in 南屯區 at 臺中市南屯區五權西路二段328號, with a published route chainage of 10.75 km in the selected direction.[^tdx-tmrt] The address is the source-backed siting description used here; no extra catchment boundary is inferred from it.
...
The operator describes the Green Line as a steel-wheel, medium-capacity, fully driverless system using two-car trains.[^tmrt-faq] It identifies G11 as an elevated station; only G0 and G17 are ground level.[^tmrt-faq] That system-level engineering fact is kept separate from the station address because TDX publishes the latter but not the former.[^tdx-tmrt][^tmrt-faq]



The operator's current public-art roster names works at selected Green Line stations, but it does not assign a named work to G11 in the roster consulted for this page.[^tmrt-art] Whether an unlisted or later work exists here is therefore TBC; the page does not turn a list boundary into a claim that no artwork exists.
...
The committed TDX station record places G11 at sequence 12 of the Green Line, in 南屯區, with a published chainage of 10.75 km for the selected direction.[^tdx-tmrt] This is a position in the operator's station order, not a travel-time or distance-between-stations claim; TDX publishes the chainage field but not a timetable for this page.[^tdx-tmrt]
