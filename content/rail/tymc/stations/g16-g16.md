---
title: "G16 — Taoyuan Green Line"
summary: "G16 (G16) on Taoyuan Metro's under-construction Green Line; location, structure and transfer evidence are kept separate from TDX's Airport MRT operating records."
line: G
operator: TYMC
updated: 2026-08-29
facts:
  ...
  - label: "Structure"
    ...
    source: dorts-green
  - label: "Location"
    value: "Dayuan District, a new road in the Aviation Industry Zone"
    source: dorts-green
  - label: "Transfer note"
    value: "TBC"
specs:
...
G16 is the station code published for this unnamed Green Line station; the project page leaves its station name open.[^dorts-green] The project table places it at Dayuan District, a new road in the Aviation Industry Zone and classifies the station as elevated; those are project-design fields, not observations from a currently operating railway.[^dorts-green]

## Place in the Green Line project

G16 sits in the Taoyuan Green Line station sequence published by the Taoyuan engineering bureau. The source table lists the code, a location description and a transfer note where one has been defined; it does not turn an unnamed station code into a final public name.[^dorts-green] No transfer is listed in the checked row.[^dorts-green]
...
The route is still under construction. A construction-project station should not be described as though it already has passenger service, a live timetable or observed daily use. The engineering bureau's current page gives the line-wide target as 2030, but it does not provide a station-specific opening date for G16; that field remains TBC here.[^dorts-green]

## Design contract, no construction contract yet

Unlike G18 and G19, no construction contract has been let for this station. The DORTS project table lists a detailed-design consulting contract covering the section between G14 (exclusive) and G17 (exclusive) — which includes G16 — with cumulative progress of 80.14% as of March 2026.[^dorts-green] This is a design-phase figure, not evidence of any civil works underway; a separate secondary account corroborates that no construction tender had been issued for this section as of mid-2025, consistent with a project still in detailed design rather than construction.[^dorts-green]
