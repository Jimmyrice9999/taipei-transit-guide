---
title: Station naming and renames
summary: Officially documented former names, appended station names and the gaps where a current label is not yet a rename record.
order: 3
updated: 2026-08-23
facts: []
specs: []
sources:
  - id: dorts-naming-guideline
    title: Taipei Metro station planning and design practice, p. 109
    titleOriginal: 捷運工程叢書精進版－2 捷運車站規劃與設計實務
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://ebook.dorts.gov.taipei/ebook/no2/files/basic-html/page109.html
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official page recording the 2007 naming, renaming and appended-name procedure.
  - id: taipei-naming-rules-2019
    title: Rules for naming, renaming and appending names to Taipei metropolitan rapid-transit stations
    titleOriginal: 臺北市臺北都會區大眾捷運系統捷運車站命名更名或車站站名加註名稱作業要點
    publisher: Taipei City Government Department of Legal Affairs (臺北市政府法務局)
    url: https://laws.gov.taipei/Law/LawSearch/LawArticleContent/FL043411
    accessed: 2026-08-23
    kind: primary
    lang: zh-Hant
    note: Full current official rule text, including the 2019 order number, responsible bodies, naming criteria, rename vote and appended-name limits.
  - id: dorts-banqiao-former-name
    title: Bannan Line CP264 completion drawings, p. 260
    titleOriginal: 捷運技術半年刊 第45期
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://ebook.dorts.gov.taipei/JRTST/ebook/no45/files/basic-html/page260.html
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official page naming 板橋站 as formerly 漢生路站.
  - id: dorts-daqiaotou-former-name
    title: Daqiaotou station land-development case
    titleOriginal: 大橋頭站捷二土地開發案
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://ebook.dorts.gov.taipei/ebook/no20/files/basic-html/page81.html
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official page saying 大橋頭站 was originally called 大橋國小站 and recording the later name change.
  - id: dorts-mingde-former-name
    title: Taipei Metro station design introduction
    titleOriginal: 捷運車站建築設計介紹
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://www.dorts.gov.taipei/cp.aspx?n=DA11BCC514603804
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official page describing 明德 as the station formerly called 天母.
  - id: dorts-daqiaotou-added
    title: Daqiaotou station and Taipei Bridge surroundings, p. 17
    titleOriginal: 捷運技術半年刊 第42期
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://ebook.dorts.gov.taipei/JRTST/ebook/no42/files/basic-html/page17.html
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official page recording the local petition and later approval to add Daqiaotou station.
  - id: ntpc-sanying-naming-gazette
    title: New Taipei City Gazette, winter issue 14, 2020
    titleOriginal: 新北市政府公報 109年冬字第14期
    publisher: New Taipei City Government (新北市政府)
    url: https://doc2-exthird.ntpc.gov.tw/NTPC_SODPUBLISH/FILE/0000000/PUBLISH/109/2/109214.pdf
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official gazette notice changing LB03 from 挖子 to 長壽山 and appending 三峽廣行宮 to LB05.
  - id: ntmc-sanying-route
    title: Sanying Line route and station information
    titleOriginal: 三鶯線路網圖與工程站點
    publisher: New Taipei City Department of Rapid Transit Systems (新北市政府捷運工程局)
    url: https://www.dorts.ntpc.gov.tw/about/routeInfo/1MJjdJr7dAv6
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full current official route page publishing LB03 長壽山 and LB05 龍埔 with its appended name.
  - id: ntpc-lb08-appended-gazette
    title: New Taipei City Gazette, autumn issue 12, 2025
    titleOriginal: 新北市政府公報 114年秋字第12期
    publisher: New Taipei City Government (新北市政府)
    url: https://doc2-exthird.ntpc.gov.tw/NTPC_SODPUBLISH/FILE/0000000/PUBLISH/114/3/114312.pdf
    accessed: 2026-08-23
    kind: primary
    lang: zh-Hant
    note: Full official gazette, p. 30, publishing the 8 September 2025 action that appends 新北市美術館 to LB08.
  - id: dorts-circular-planning-name
    title: Circular Line Y16 station design, p. 235
    titleOriginal: 捷運技術半年刊 第45期
    publisher: Taipei City Department of Rapid Transit Systems (臺北市政府捷運工程局)
    url: https://ebook.dorts.gov.taipei/JRTST/ebook/no45/files/basic-html/page235.html
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full official planning page using the label Y16車站（民生新埔站）.
  - id: ntmc-circular-stations
    title: Circular Line station list
    titleOriginal: 環狀線車站
    publisher: New Taipei Metro Corporation (新北大眾捷運股份有限公司)
    url: https://www.ntmetro.com.tw/basic/?node=10138
    accessed: 2026-08-17
    kind: primary
    lang: zh-Hant
    note: Full operator station list publishing Y17 as 新埔民生.
---

Station names have two different kinds of official record: a naming action, and
a later page that remembers or uses a name. DORTS says that naming, renaming and
appended names are meant to follow a defined procedure established in 2007.[^dorts-naming-guideline]

## How the formal process works

The current Taipei rules are the text issued under order
府捷秘字第10830001521號 on 17 January 2019.[^taipei-naming-rules-2019] They assign
names for stations under construction to DORTS and names for operating stations
to TRTC, with a standing review committee chaired by the DORTS commissioner.[^taipei-naming-rules-2019]
Candidate names are assessed for local identity, landmark prominence and
historical meaning; the named feature must be within 500 metres, names are
limited to six Chinese characters, and duplicate or confusingly similar names
are barred.[^taipei-naming-rules-2019]

For a proposed Taipei rename, the 2019 rules require signatures from at least
8% of eligible residents in the station's own and adjacent neighbourhoods.[^taipei-naming-rules-2019]
After review and verification, a proposal passes only if yes votes outnumber no
votes and reach one quarter of all eligible voters; a failed proposal cannot be
resubmitted for that station for four years.[^taipei-naming-rules-2019] An
appended name is a different instrument: it supplements an insufficiently
distinct station name, is normally limited to one addition within 800 metres
and six years, and does not appear on station entrances.[^taipei-naming-rules-2019]

## Former names recorded by DORTS

| Station | Former name | What the primary page establishes |
|---|---|---|
| BL07 | 漢生路站 | DORTS calls current 板橋站 「原名漢生路站」; the action date and instrument are TBC.[^dorts-banqiao-former-name] |
| O12 | 大橋國小站 | DORTS calls 大橋頭站 「原稱大橋國小站」 and records the later change; the action date and instrument are TBC.[^dorts-daqiaotou-former-name] |
| R18 | 天母站 | DORTS describes 明德 as 「原名天母站」; the action date and instrument are TBC.[^dorts-mingde-former-name] |

These former-name records are not inferred from slash notation or from a
planning code; each appears as a former-name statement on a fetched official
page.[^dorts-banqiao-former-name][^dorts-daqiaotou-former-name][^dorts-mingde-former-name]

## Formal rename and appended name

The 9 June 2020 New Taipei gazette says LB03站：挖子，更名為「長壽山」.[^ntpc-sanying-naming-gazette]
The current New Taipei DORTS route page uses **LB03 長壽山**.[^ntmc-sanying-route]

The same gazette says LB05站：龍埔，加註「三峽廣行宮」; this is an appended
name, not a rename of 龍埔.[^ntpc-sanying-naming-gazette] The current route page
preserves that distinction as **LB05 龍埔（加註名稱：三峽廣行宮）**.[^ntmc-sanying-route]

New Taipei later used the same type of action for LB08. Its 8 September 2025
gazette says LB08站：加註「新北市美術館」 and identifies the legal basis as the
New Taipei naming, renaming and appended-name rules.[^ntpc-lb08-appended-gazette]
The notice calls this an addition to the station name, so it is not presented
here as a replacement for the station's primary name.[^ntpc-lb08-appended-gazette]

## Planning labels and current names

DORTS’s Circular Line planning page calls Y16 「民生新埔站」, while the current
operator station list publishes Y17 as **新埔民生**.[^dorts-circular-planning-name][^ntmc-circular-stations]
No naming action was found for this order change, so it remains a published
label conflict rather than a claimed rename.

Daqiaotou’s addition is a separate history: DORTS says it was not in the
original Executive Yuan-approved Xinzhuang Line station list, but was added
after local petitions and approval to use Daqiao Elementary land.[^dorts-daqiaotou-added]

## Gaps

The gazette/order dates for the BL07, O12 and R18 former names remain TBC, as do
the official instruments behind the failed leads recorded in the research
report. Current station lists and engineering labels can establish what a
source called a station; they cannot, by themselves, establish when a formal
rename occurred.[^dorts-naming-guideline]
