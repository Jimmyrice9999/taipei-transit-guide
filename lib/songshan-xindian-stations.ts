...
const gData: Record<string, GData> = {
  ...
  G18: {
    ...
    engineeringHistorySource: dortsSongshanArchitecture,
    proseSources: [dortsSongshanG18Architecture, dortsSongshanArchitectureIntro, dortsSongshanArchitecturePrinciples, dortsSongshanStationTable],
    prose: [
      ...
      { text: 'The same full architecture page labels the station G21 in its source-era sequence, while the current operator page identifies Nanjing Sanmin as G18.', source: dortsSongshanG18Architecture.id },
      { text: 'Those identifiers are retained as different records: G21 is the builder’s engineering-era label and G18 is the current operating code.', source: trtcStation('G18').id },
      { text: 'The station sits at the Nanjing East Road and Sanmin Road intersection, which DORTS treats as a principal approach into Taipei from the Songshan district.', source: dortsSongshanG18Architecture.id },
      { text: 'Its office buildings form an orderly street wall rather than the mixed historic fabric described for the western stations of the line.', source: dortsSongshanG18Architecture.id },
      { text: 'The continuous arcades are part of that street condition, so the station’s public edge is read together with the covered pedestrian frontage.', source: dortsSongshanG18Architecture.id },
      { text: 'DORTS identifies commuting and school travel as the dominant use pattern in this setting, with shopping and tourism secondary.', source: dortsSongshanG18Architecture.id },
      { text: 'It therefore frames the station as a daily neighbourhood threshold as well as a route into the city.', source: dortsSongshanG18Architecture.id },
      { text: 'The source describes the design idea as a community vestibule and city gateway, using the meaning of a vestibule as a transition into another space.', source: dortsSongshanG18Architecture.id },
      { text: 'That interpretation gives the station a civic role without claiming that its surrounding offices form a separate commercial centre.', source: dortsSongshanG18Architecture.id },
      { text: 'The wider Songshan Line runs from Ximen to Songshan through seven underground stations and was planned as a route through different historical, cultural and environmental settings.', source: dortsSongshanArchitectureIntro.id },
      { text: 'DORTS says the stations were given local themes and identities because their transport functions and surrounding conditions differ.', source: dortsSongshanArchitectureIntro.id },
      { text: 'For Nanjing Sanmin, the local evidence is the office-and-arcade corridor and its everyday commuter and school demand, not a historic monument or a large event venue.', source: dortsSongshanG18Architecture.id },
      { text: 'The line-wide design principles require circulation to be simple and clear while using modular and systematised planning methods.', source: dortsSongshanArchitectureIntro.id },
      { text: 'They also bring entrances, ventilation structures, landscape, seating, interior finishes and lighting into one architectural and public-art vocabulary.', source: dortsSongshanArchitecturePrinciples.id },
      { text: 'The principles require the resulting treatment to account for passenger movement, visual experience, safety, material durability and maintenance.', source: dortsSongshanArchitecturePrinciples.id },
      { text: 'The fetched primary pages do not establish a G18 civil contract, final account, rename gazette, complete artwork record or paid-area versus street-transfer classification.', source: dortsSongshanArchitecturePrinciples.id },
      { text: 'Those fields remain TBC rather than being inferred from the station’s current facilities or its office-district setting.', source: dortsSongshanArchitecturePrinciples.id },
    ],
  },
  G19: {
    ...
    engineeringHistorySource: dortsSongshanArchitecture,
    proseSources: [dortsSongshanArt, dortsSongshanArchitectureIntro, dortsSongshanArchitecturePrinciples, dortsSongshanStationTable],
    prose: [
      ...
      { text: 'The DORTS station table labels this station G22 in its source-era engineering sequence, while the current operator page identifies Songshan as G19.', source: dortsSongshanStationTable.id },
      { text: 'The two labels are retained with their eras rather than reconciled: G22 belongs to the builder’s technical record and G19 to the current operating registry.', source: trtcStation('G19').id },
      { text: 'The station occupies a transport junction where railway and road movements meet, and DORTS describes it as a centre of Songshan’s living, religious, commercial and administrative activity.', source: dortsSongshanArt.id },
      { text: 'The area remains busy through the day and changes into a brightly lit commercial street at night, which is why the architecture article names a festival-of-light theme.', source: dortsSongshanArt.id },
      { text: 'The surrounding district retains local cultural events, including the old Xikou music festival and other folk celebrations.', source: dortsSongshanArt.id },
      { text: 'DORTS connects the station’s identity to the former name Malisikou, which refers to a bend in the river and recalls the water-transport trade that helped Songshan prosper.', source: dortsSongshanArt.id },
      { text: 'The same account describes the district’s later transformation into a major transport and economic node, so the public-art brief uses a historical name as an index to changing urban life.', source: dortsSongshanArt.id },
      { text: 'The work was selected through an open call, and the record names the artist group 麻粒國際文化試驗 and the work 河流彎曲之處，域見繁花光穹.', source: dortsSongshanArt.id },
      { text: 'DORTS says the artists began from the idea of lighting the environmental image and the viewer’s imagination, using a large light sculpture to create a garden-like image in public space.', source: dortsSongshanArt.id },
      { text: 'This is a documented procurement and site-response history, not a claim that the public artwork represents every cultural activity around the station.', source: dortsSongshanArt.id },
      { text: 'The wider Songshan Line was designed to connect the Ximen and Songshan commercial centres and to make the shift between older and newer urban economies legible.', source: dortsSongshanArchitectureIntro.id },
      { text: 'Its architectural principles require public art and station design to be developed together from the basic-design stage.', source: dortsSongshanArchitectureIntro.id },
      { text: 'Those principles extend from entrances and ventilation structures to landscape, seating, floors, walls, ceilings and lighting, with passenger movement and maintenance treated as design constraints.', source: dortsSongshanArchitecturePrinciples.id },
      { text: 'The fetched primary pages do not establish a G19 civil contract, final account, rename gazette, artwork completion year or fare-boundary classification.', source: dortsSongshanArt.id },
      { text: 'Those fields remain TBC rather than being filled from the station’s landmark list or from the public-art title.', source: dortsSongshanArt.id },
    ],
