...
const dortsNangangTrackWorksite = source(
  'dorts-cn531-track-worksite-p241',
  'Nangang East Extension track worksite scope',
  '軌道工程施工管理之探討─以南港線東延段為例',
  dortsPublisher,
  'https://ebook.dorts.gov.taipei/JRTST/ebook/no39/files/basic-html/page241.html',
  'The builder’s full technical-journal page publishes the CE731 contractor, the source-era Kunyang–Nangang–Exhibition Hall scope and its approximately 2,913-metre track work.',
  '2026-08-18',
)
...
const dortsNangangCutCover = source(
  ...
  'The builder’s full tunnel-engineering page explains shield and cut-and-cover methods, identifies Sun Yat-Sen Memorial Hall and City Hall as sequential cut-and-cover examples using struts, and names Houshanpi and Yongchun as reverse-method examples.',
  '2026-08-18',
)
...
const stationProse: Record<string, StationProseSentence[]> = {
  ...
  BL19: [
    { text: 'Yongchun is part of DORTS’s approximately 10.3-kilometre, entirely underground Nangang section, planned through a dense and high-traffic commercial corridor along the Zhongxiao East Road spine.', source: dortsNangang.id },
    { text: 'The wider CN531 track article reports 30.6 kilometres and twelve mainline stations with both ballasted and ballastless track, a line-wide contract scope rather than a length for this station.', source: dortsNangangTrack.id },
    { text: 'DORTS records eleven failed or aborted CN531/CP541 tenders before the ROC85 October award to DeSheng and Korean Wido.', source: dortsNangangTrack.id },
    { text: 'The published amounts, NT$1,309,170,114 for CN531 and NT$488,829,886 for CP541, measure track packages and do not establish Yongchun’s civil station price.', source: dortsNangangTrack.id },
    { text: 'The procurement strategy separates international joint bidding for CN253B and CN257 from domestic-led CN253A, CN254 and CN256 packages supported by foreign technical cooperation.', source: dortsProcurementStrategy.id },
    { text: 'It also records negotiated procurement with the Veterans Engineering Office for CN251 and CN252 because the Boai-district alignment passed beside sensitive government and civic buildings.', source: dortsProcurementStrategy.id },
    { text: 'DORTS’s tunnel-engineering record names Yongchun and Houshanpi as examples of the reverse construction method, a source-specific method classification rather than a claim that every nearby structure used the same sequence.', source: dortsNangangCutCover.id },
    { text: 'The Nankang architecture guidance says the line sought to reduce the visual mass of surface facilities, connect underground activity with the city and use public art to create landmarks.', source: dortsArchitecture.id },
    { text: 'The public-art planning chapter gives City Hall, Kunyang and Zhongxiao Dunhua as selected locations for special urban or spatial reasons, but supplies no Yongchun-specific procurement record.', source: dortsNangangArtPlanning.id },
    { text: 'The flood-design record documents a post-Nari freeboard revision from 0.5 metre to 1.1 metres and publishes historical design tables for engineering review.', source: dortsNangangFloodResponse.id },
    { text: 'Those tables use historical DORTS engineering labels that the source distinguishes from TRTC operating codes, so no flood value is silently reassigned to current BL19.', source: dortsNangangFloodResponse.id },
    { text: 'The reviewed primary pages do not settle Yongchun’s station-civil contractor and final account, construction-era name or rename gazette, paid-area transfer classification, complete public-art record or depot roster; those fields remain TBC.', source: dortsNangang.id },
  ],
  BL20: [
    { text: 'Houshanpi lies in the underground Nangang section that DORTS describes as an approximately 10.3-kilometre, eleven-station corridor inserted into a dense commercial road environment.', source: dortsNangang.id },
    { text: 'The track journal gives the broader CN531 package a different scope of 30.6 kilometres and twelve mainline stations, including both ballasted and ballastless track, and that value is not treated as a station measurement.', source: dortsNangangTrack.id },
    { text: 'The CN531/CP541 works endured eleven failed or aborted tenders before the ROC85 October award to DeSheng and Korean Wido.', source: dortsNangangTrack.id },
    { text: 'DORTS publishes NT$1,309,170,114 for CN531 and NT$488,829,886 for CP541; these are line-wide track-contract amounts rather than Houshanpi civil costs.', source: dortsNangangTrack.id },
    { text: 'The procurement record assigns CN253B and CN257 to international joint bidding, CN253A/CN254/CN256 to domestic contractors with foreign technical cooperation, and CN251/CN252 to negotiated procurement in the Boai district.', source: dortsProcurementStrategy.id },
    { text: 'DORTS’s full tunnel-method page names Houshanpi and Yongchun as reverse-method construction examples, preserving a technical distinction from the sequential cut-and-cover examples named for other Nangang stations.', source: dortsNangangCutCover.id },
    { text: 'That source does not publish Houshanpi’s excavation dimensions, contractor or final account, so the method label is not expanded into an unsupported station specification.', source: dortsNangangCutCover.id },
    { text: 'The architecture record describes a restrained Nankang surface treatment, linked underground commercial activity and public art used to create urban landmarks without enlarging station structures.', source: dortsArchitecture.id },
    { text: 'Its public-art planning chapter identifies selected locations elsewhere in the early Nankang programme and does not provide a Houshanpi-specific artwork procurement record.', source: dortsNangangArtPlanning.id },
    { text: 'The flood-engineering source records the post-Nari freeboard change from 0.5 metre to 1.1 metres and warns that DORTS engineering-era labels differ from TRTC’s operating-code system.', source: dortsNangangFloodResponse.id },
    { text: 'For that reason, historical table rows are not silently mapped to current BL20, and the page does not present a source-era flood number as a current station fact.', source: dortsNangangFloodResponse.id },
    { text: 'The fetched primary material does not settle Houshanpi’s station-civil contractor and final account, construction-era name or rename gazette, paid-area transfer classification, complete public-art record or depot roster; those fields remain TBC.', source: dortsNangang.id },
  ],
  BL21: [
    { text: 'Kunyang is part of DORTS’s underground Nangang corridor, while the East Extension technical record treats the Kunyang–Nangang–Exhibition Hall works as a separate construction-management challenge.', source: dortsNangang.id },
    { text: 'The track-management article records a narrow and complicated work route, incomplete civil and open-cut interfaces, a seven-month schedule pressure and the use of a specialist track subcontractor.', source: dortsNangangTrackManagement.id },
    { text: 'It says the track work began in ROC96 August and had to be coordinated with civil handover phases rather than proceeding through an uninterrupted finished route.', source: dortsNangangTrackManagement.id },
    { text: 'A companion CE731 worksite record names Alstom/CTCI and describes approximately 2,913 metres of plinth, special-track, running-rail, third-rail, buffer and turnout work across the East Extension scope.', source: dortsNangangTrackWorksite.id },
    { text: 'That article uses source-era labels BL16, BL17 and BL18 for the Kunyang–Nangang–Exhibition Hall sequence, whereas the current operating overlay uses BL21, BL22 and BL23; the identifiers are retained as different numbering systems.', source: dortsNangangTrackWorksite.id },
    { text: 'The wider CN531 history reports 30.6 kilometres and twelve mainline stations with mixed ballasted and ballastless track, while the East Extension article’s approximately 2,913 metres is a separate worksite measurement.', source: dortsNangangTrack.id },
    { text: 'The public-art planning record selected Kunyang because it was a terminal at the time and allocated a NT$5 million planning ceiling to the work.', source: dortsNangangArtPlanning.id },
    { text: 'The planning process recorded a preliminary committee meeting on 29 August and a final review on 4 September, with the interactive carousel installation completed on ROC89 December 15.', source: dortsKunyangArtProcess.id },
    { text: 'DORTS describes the installation as an interactive work whose moving and sound-producing mechanism responds to a passenger’s approach, making the artwork part of the station’s passenger movement rather than a static wall display.', source: art.bl21.id },
    { text: 'The current artwork record publishes NT$4,999,000, while the planning page publishes a NT$5 million ceiling; the values are retained as stated programme-stage ceiling and later artwork expenditure.', source: art.bl21.id },
    { text: 'The flood-design record documents the post-Nari freeboard revision from 0.5 metre to 1.1 metres, but its historical DORTS station identifiers are not silently converted into current TRTC operating codes.', source: dortsNangangFloodResponse.id },
    { text: 'The reviewed records do not settle a Kunyang station-civil contractor and final account, rename gazette, paid-area transfer classification or station-level depot roster; those fields remain TBC.', source: dortsNangang.id },
  ],
  BL22: [
    { text: 'Nangang is the East Extension station for which DORTS’s architecture page records co-development with Taiwan Railways and a design intended to connect local historical references with contemporary transport technology.', source: dortsStationArchitecture.id },
    { text: 'The page identifies a light-green theme and names 新庄仔陂, 後山陂, 舜若多橋, Nangang tea gardens and the Hu Shih Memorial Hall among the local references used in the design.', source: dortsStationArchitecture.id },
    { text: 'The East Extension track-management article describes a narrow, complex work route, unfinished civil and open-cut areas and a seven-month schedule pressure requiring phased handover and specialist track work.', source: dortsNangangTrackManagement.id },
    { text: 'The CE731 worksite record names Alstom/CTCI and publishes approximately 2,913 metres of plinth, special-track, running-rail, third-rail, buffer and turnout scope connecting the source-era Kunyang–Nangang–Exhibition Hall sequence.', source: dortsNangangTrackWorksite.id },
    { text: 'Because that record calls the three stations BL16, BL17 and BL18 while the current operating overlay calls them BL21, BL22 and BL23, the construction identifiers are published as source-era labels rather than silently normalised.', source: dortsNangangTrackWorksite.id },
    { text: 'The route and track records therefore describe different measurements: the wider CN531 article reports 30.6 kilometres and twelve mainline stations, while the East Extension worksite reports approximately 2,913 metres for one construction scope.', source: dortsNangangTrack.id },
    { text: 'DORTS’s architecture record treats the station as a co-development interface with Taiwan Railways, not merely as a freestanding underground passenger box.', source: dortsStationArchitecture.id },
    { text: 'The same record links the station’s palette and motifs to Nangang’s local water, bridge, tea and memorial references, but does not publish a station civil contract value.', source: dortsStationArchitecture.id },
    { text: 'The current public-art record is retained in the facts box, while this batch does not infer additional work from the station’s architecture themes or from neighbouring station entries.', source: art.bl22.id },
    { text: 'DORTS’s flood-engineering page records a post-Nari freeboard revision from 0.5 metre to 1.1 metres and distinguishes its historical engineering labels from TRTC’s current operating codes.', source: dortsNangangFloodResponse.id },
    { text: 'The historical flood table is consequently not used as an unqualified current BL22 station lookup.', source: dortsNangangFloodResponse.id },
    { text: 'The fetched primary material does not settle Nangang’s station-civil contractor and final account, construction-era name or rename gazette, paid-area transfer classification, complete public-art record or station-level depot roster; those fields remain TBC.', source: dortsNangangEast.id },
  ],
  BL23: [
    { text: 'Taipei Nangang Exhibition Center is the eastern end of the DORTS East Extension sequence whose station-architecture record describes a digital-space, e-station concept.', source: dortsStationArchitecture.id },
    { text: 'DORTS records a dawn-blue palette, metal and low-E glass, shell-like entrances, and barcode, circuit-board and binary motifs as the architectural vocabulary for that concept.', source: dortsStationArchitecture.id },
    { text: 'The East Extension track-management article describes work through narrow and complicated construction areas, incomplete civil and open-cut interfaces, phased handover and a seven-month schedule pressure.', source: dortsNangangTrackManagement.id },
    { text: 'The CE731 worksite article names Alstom/CTCI and gives approximately 2,913 metres for plinth, special-track, running-rail, third-rail, buffer and turnout works extending through the source-era Kunyang–Nangang–Exhibition Hall scope.', source: dortsNangangTrackWorksite.id },
    { text: 'Its BL16, BL17 and BL18 labels are source-era engineering identifiers, while the current operating station registry uses BL21, BL22 and BL23; the two sequences are not silently merged.', source: dortsNangangTrackWorksite.id },
    { text: 'The wider CN531 record reports 30.6 kilometres and twelve mainline stations with both ballasted and ballastless track, so its figure and the East Extension’s 2,913 metres measure different published scopes.', source: dortsNangangTrack.id },
    { text: 'The current DORTS public-art record describes two separately selected works at the station, with one obtained through an invited competition and the other through public selection; this is a procurement distinction, not an inference from their titles.', source: art.bl23.id },
    { text: 'The same record places one work in the concourse and the other along the paid-area passage toward the Wenhu Line, giving the station’s public art both a central passenger-space and an interchange-corridor role.', source: art.bl23.id },
    { text: 'DORTS’s architecture page describes the station as an attempt to make digital imagery and local movement legible in the material environment, while the artwork record separately documents the installed works and their procurement.', source: dortsStationArchitecture.id },
    { text: 'The route and East Extension records do not publish a station-specific civil contractor and final account, so Alstom/CTCI is not presented as the civil station builder.', source: dortsNangangTrackWorksite.id },
    { text: 'The flood-design source records a post-Nari increase in freeboard from 0.5 metre to 1.1 metres and warns that historical DORTS engineering identifiers differ from current TRTC operating codes.', source: dortsNangangFloodResponse.id },
    { text: 'No historical flood-table row is therefore silently reassigned to current BL23.', source: dortsNangangFloodResponse.id },
    { text: 'The reviewed primary sources do not settle a construction-era name or rename gazette, paid-area transfer classification or station-level depot roster; those fields remain TBC.', source: dortsNangangEast.id },
  ],
}
