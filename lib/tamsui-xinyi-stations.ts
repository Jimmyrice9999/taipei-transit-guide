...
const rData: Record<string, RData> = {
  ...
  R27: {
    ...
    proseSources: [dortsRoute, dortsGroundStationTypes, dortsRailwayCorridorPlanning, dortsStationDesignGoals, dortsNorthernPublicRealm, hongshulinReserve, ntpcDanhaiLrtOpening],
    prose: [
      { text: 'DORTS places Hongshulin in the ground-level section from Fuxinggang to Hongshulin, the part of the Tamsui–Xinyi Line where track and platforms remain at grade.', source: dortsGroundStationTypes.id },
      { text: 'The department calls that form economical but warns that a surface railway can interrupt movement and views, making its relationship with public space a design issue as well as a construction choice.', source: dortsGroundStationTypes.id },
      { text: 'The former TRA Tamsui branch right-of-way was obtained through a paid transfer, and DORTS says that reusing it made planning changes and land acquisition less difficult.', source: dortsRailwayCorridorPlanning.id },
      { text: 'The same corridor record says the former railway already carried high northern demand and that converting it to metro was expected to improve economic benefit.', source: dortsRailwayCorridorPlanning.id },
      { text: 'DORTS’s northern public-realm account treats the inherited route as a recreational resource, with walking and cycling paths, planting and small plazas used to soften the infrastructure’s visual effect.', source: dortsNorthernPublicRealm.id },
      { text: 'The Hongshulin reserve page directs visitors to the reserve from exit 2 of Hongshulin station, making the station a documented access point to the protected riverside landscape.', source: hongshulinReserve.id },
      { text: 'That government page places the reserve in the Tamsui district and presents it within New Taipei’s Golden Waterfront public-realm system.', source: hongshulinReserve.id },
      { text: 'Hongshulin is also a cross-system interface: New Taipei’s official account identifies it as the origin of the Green Mountain Line, which opened on 24 December 2018 and runs to Kanding with eleven stations.', source: ntpcDanhaiLrtOpening.id },
      { text: 'The same account describes the Green Mountain route as elevated from Hongshulin to Tamsui Administrative Center and at grade farther into the new town, so the interchange joins two different light-rail settings.', source: ntpcDanhaiLrtOpening.id },
      { text: 'DORTS’s route record and the current TRTC station page identify the Hongshulin–Danhai connection, but the fetched sources do not establish whether the transfer is paid-area or street-side.', source: dortsRoute.id },
      { text: 'The station-design brief asks each station to have a distinct character, integrate with its surroundings and remain a durable modern work rather than a short-lived stylistic object.', source: dortsStationDesignGoals.id },
      { text: 'For R27, the sources therefore establish a railway-corridor, ecological-access and light-rail-interchange context without inventing a station-specific construction anecdote.', source: hongshulinReserve.id },
      { text: 'The fetched primary pages do not identify R27’s individual architect, civil-contract award value, final account, completed artwork register or legal rename.', source: dortsRoute.id },
      { text: 'Those fields remain TBC pending the period procurement, completion, artwork and gazette records.', source: dortsStationDesignGoals.id },
    ],
  },
  R28: {
