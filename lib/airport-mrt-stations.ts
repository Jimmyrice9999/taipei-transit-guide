...
const stationProse: Record<string, StationProseSentence[]> = {
  ...
  A13: [
    { text: 'The Railway Bureau\'s CU02 package grouped A13 with A12, A14 and A14a for station structures, architectural fit-out and water works; the 102 annual report says those structures and fit-out were complete, and that equipment testing and BMS integration had been carried out.', source: rbAnnual102.id },
    { text: 'CU02A separately covered the airport underground section from A12 to A14a, including cut-and-cover retaining and support works, deep excavation, shield tunnelling, internal tunnel structures and an expansion of the existing tunnel beside Terminal 2\'s east parking area.', source: rbAnnual102.id },
    { text: 'The report gives CU02A an approximately 5,520-metre scope and records Railway Bureau acceptance on 25 September 2012; that is a package measurement and acceptance date, not an A13-only construction value.', source: rbAnnual102.id },
    { text: 'The airport-risk paper places this underground work beneath sensitive airport interfaces, including the runway and control-tower zone, and describes the A12-A14a ground profile used for monitoring as a thin red-soil layer over gravel-bearing strata with shallow groundwater.', source: rbAirportRisk.id },
    { text: 'The 102 report records 99.83% year-end progress for the whole CU02 package, which includes four stations, rather than for A13 alone.', source: rbAnnual102.id },
    { text: 'The Railway Bureau architecture report says the Airport MRT public-art programme used commissioned creation for the overall design and open selection for planning, design supervision and installation services, but it does not publish an A13 station-only cost.', source: rbStations.id },
  ...
  A14a: [
    { text: 'A14a belongs to CU02, the package for the four underground airport stations from A12 through A14a; the 102 annual report records complete station structures and architectural fit-out, completed equipment installation and testing, and BMS and SCADA interface work for the package.', source: rbAnnual102.id },
    { text: 'The related CU02A package extended from A12 to A14a and combined shield tunnelling with the airport cut-and-cover and deep-excavation works, including retaining walls, supports, base excavation and internal tunnel structures.', source: rbAnnual102.id },
    { text: 'CU02A is reported at approximately 5,520 metres and was accepted by the Railway Bureau on 25 September 2012; the report does not break that length or acceptance into a station-only A14a measure.', source: rbAnnual102.id },
    { text: 'The airport-risk paper describes the same airport underground corridor as a monitored shield-tunnel and cut-and-cover environment, with red soil, gravel-bearing strata and groundwater conditions documented for the A12-A14a section.', source: rbAirportRisk.id },
    { text: 'The 102 annual report gives 99.83% progress for CU02 as a whole at year-end, so it cannot be read as an A14a completion percentage.', source: rbAnnual102.id },
    { text: 'The architecture report says the A14a landscape was coordinated with the China Airlines Park plan, retained the windbreak character of the existing tree belt, and used a paved entrance plaza, layered planting and focused lighting for the approach and gathering areas.', source: rbStations.id },
    { text: 'The checked construction reports do not publish a CU02 contractor, a station-only award value or a final A14a civil account, so those fields remain TBC.', source: rbAnnual102.id },
  ...
  A15: [
    { text: 'A15 was built under CE03A, the package for the elevated bridge from Dayuan to Qingpu and the A15 and A16 elevated stations.', source: rbAnnual102.id },
    { text: 'The CE03A description separates the route\'s out-of-ground section from its viaduct: the cut-and-cover section was complete, and both the bridge substructure and superstructure, including foundations, piers, parapets, noise barriers and cable trenches, were complete in the 102 report.', source: rbAnnual102.id },
    { text: 'The same report says both stations had completed structural work, architectural fit-out and rain-protection facilities; A15-A16 low-voltage power was supplied in August 2011, BMS integration testing finished in May 2013, and SCADA testing finished in November 2013.', source: rbAnnual102.id },
    { text: 'CE03A stood at 99.96% progress at the end of 2013, a package snapshot covering the two stations and the Dayuan-Qingpu elevated section rather than an A15-only completion figure.', source: rbAnnual102.id },
    { text: 'The DE03 architecture report treats A15 as a rural-field station near the airport and places it within a six-station elevated design group whose theme turns the two travel directions into an upward and downward movement.', source: rbStations.id },
    { text: 'Because A15 lies inside the airport lighting-restriction zone and has a narrow site, the landscape design centred the entrance, linked green belts on both sides, limited planting to species suited to the small footprint and used reflected light to reduce surrounding impact.', source: rbStations.id },
    { text: 'The checked annual report does not identify a CE03A contractor, contract value or A15-only final account, and the checked TYMC art table has no A15 row; those are not treated as proof that no artwork exists.', source: rbAnnual102.id },
  ...
  A16: [
    { text: 'The station is part of CE03A, whose scope joined the Dayuan-to-Qingpu elevated bridge with A15 and A16, so the construction record describes A16 together with its corridor rather than as an isolated work.', source: rbAnnual102.id },
    { text: 'The 102 annual report records the CE03A cut-and-cover out-of-ground section as complete and the elevated bridge foundations, piers, parapets, noise barriers and cable trenches as complete, followed by station architectural and rain-protection completion at both A15 and A16.', source: rbAnnual102.id },
    { text: 'For the station systems, the report records low-voltage power in August 2011, BMS integration in May 2013, and ME01 SCADA testing in November 2013 before whole-line system integration; CE03A was 99.96% complete at year-end 2013.', source: rbAnnual102.id },
    { text: 'The architecture report reads A16 through its rural setting: the open plaza was intended to provide public space and disaster evacuation capacity while also buffering noise and creating a transition between the road and the fields.', source: rbStations.id },
    { text: 'Its landscape brief specifies a reflective-glass theme wall, diagonal paving at the entrance and a planting scheme built around blue jacaranda, fragrant maple, chinaberry and other seasonal vegetation; lighting focuses on the concourse and vertical circulation.', source: rbStations.id },
    { text: 'The checked TYMC public-art table has no A16 allocation row, which records only what was not found in that page and does not establish that the station has no artwork.', source: publicArt.id },
    { text: 'The checked construction report does not publish a CE03A contractor, package value or station-only final account for A16, so those fields remain TBC.', source: rbAnnual102.id },
  ...
  A17: [
    { text: 'A17 was built under CE03B, the package for the Qingpu-to-Zhongli elevated bridge, the four elevated stations A17 through A20 and the Qingpu depot administration building.', source: rbAnnual102.id },
    { text: 'By the 102 report, CE03B\'s bridge foundations, piers, parapets, noise barriers and cable trenches were complete, while all four stations had completed structural work and rain-protection facilities and were finishing ceiling and architectural work.', source: rbAnnual102.id },
    { text: 'The package report records BMS integration for A17-A20 in June 2013, the start of fire-safety checks in August, completion of low-voltage power and permanent-water arrangements, and continuing SCADA and whole-line system-integration work; the package stood at 99.95% at year-end 2013.', source: rbAnnual102.id },
    { text: 'The DE03 landscape brief gives A17 a six-metre setback and a linear open space intended to serve disaster evacuation, pedestrian circulation, noise separation and rest, with a central entrance and reserved taxi-transfer space.', source: rbStations.id },
    { text: 'The architecture report places A17 within the HSR special district rather than describing it as an airport-terminal station, so its corridor role is tied to the planned new-town and HSR-area development context.', source: rbStations.id },
    { text: 'The checked TYMC public-art table has no A17 allocation row, which is a search result for that page and not evidence that no artwork exists at the station.', source: publicArt.id },
    { text: 'The checked construction report does not publish a CE03B contractor, contract value or station-only final account for A17, so those fields remain TBC.', source: rbAnnual102.id },
  ...
  A18: [
    { text: 'A18 is part of CE03B, the Qingpu-to-Zhongli elevated-bridge and four-station package that also includes A17, A19, A20 and the Qingpu depot administration building.', source: rbAnnual102.id },
    { text: 'The 102 report records the CE03B bridge structure as complete, all four station structures and rain-protection facilities complete, and ceiling fit-out in its closing phase; A17-A20 BMS integration finished in June 2013 and the package was at 99.95% progress at year-end.', source: rbAnnual102.id },
    { text: 'The same construction record says A18\'s HSR-station plaza was reported complete on 15 October 2013, while the station group continued fire-safety, SCADA and whole-line system-integration work.', source: rbAnnual102.id },
    { text: 'The station landscape brief treats A18 as a major transfer point and gateway, using a plaza-like green space beside the HSR station, diagonal paving that directs visitors toward the entrance and shaded planting that ties the plaza to the existing HSR setting.', source: rbStations.id },
    { text: 'The Railway Bureau selected A18 among eight station areas for the public-art programme; it records commissioned creation for the overall line-wide design and open selection for the planning, design supervision and installation service.', source: rbStations.id },
    { text: 'The checked construction report does not publish a CE03B contractor, contract value or station-only final account for A18, so those fields remain TBC.', source: rbAnnual102.id },
