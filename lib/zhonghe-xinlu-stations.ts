...
const stationProse: Record<string, StationProseSentence[]> = {
  ...
  O51: [
    { text: 'The station belongs to a Luzhou branch whose shared visual language takes the former district waterland and white egret as its starting point.', source: architecture.o51.id },
    { text: 'For O51, the local variation is the isolated sandbar, expressed through floor patterns that make the principal circulation route appear to pass around a sandbank.', source: architecture.o51.id },
    { text: 'The page describes square forms as a way to express the granular quality of sand, so the floor treatment is both a surface pattern and a directional device.', source: architecture.o51.id },
    { text: 'The entrance imagery carries the waterbird vocabulary above ground, connecting the visible station elements to the interior theme instead of treating them as unrelated street furniture.', source: architecture.o51.id },
    { text: 'Wall panels continue the sandbar reference at platform level, while the seating areas use a brown theme colour to keep the material palette legible.', source: architecture.o51.id },
    { text: 'DORTS presents this integration as an architectural treatment even though the page says that a separately selected public-art work was not commissioned for the station.', source: architecture.o51.id },
    { text: 'That distinction matters because a design concept, a built decorative treatment and a commissioned artwork are different records in the department’s material.', source: architecture.o51.id },
    { text: 'The general station-architecture guidance asks each station to have its own character while remaining part of a clear network image.', source: dortsStationArchitectureGeneral.id },
    { text: 'It also asks station design to integrate with surrounding buildings and to use contemporary materials and construction methods to express local identity.', source: dortsStationArchitectureGeneral.id },
    { text: 'O51 therefore reads in the source as a branch-specific interpretation of a common egret vocabulary, not as an isolated decorative scheme.', source: architecture.o51.id },
    { text: 'The fetched DORTS records identify the design treatment but do not name an individual architect, civil-contract value, final account or legal station rename.', source: architecture.o51.id },
    { text: 'A complete current artwork register and any gazette establishing a naming change remain TBC rather than being inferred from the architecture page.', source: dortsStationArchitectureGeneral.id },
  ...
  O52: [
    { text: 'DORTS gives St. Ignatius High School station the public-art theme Reeds under the Moon and makes reeds and white egrets its principal design elements.', source: architecture.o52.id },
    { text: 'The page frames the theme through the changing position of the moon and reeds, using the habitat of waterbirds as the bridge between the Luzhou branch and this station’s local variation.', source: architecture.o52.id },
    { text: 'That concept is carried into both the station’s hardware and software, meaning the entrances, ventilation structures, finishes and images are treated as parts of one visual idea.', source: architecture.o52.id },
    { text: 'The entrance and ventilation forms repeat the branch’s white-egret vocabulary while keeping the above-ground elements consistent across the contract section.', source: architecture.o52.id },
    { text: 'DORTS records green as the station’s theme colour, giving the reed and wetland references a distinct chromatic identity within the branch.', source: architecture.o52.id },
    { text: 'Mosaic work at the entrance turns the bird image into a surface treatment rather than presenting a freestanding object at the street edge.', source: architecture.o52.id },
    { text: 'Inside, horizontal and vertical floor lines are used to suggest reeds, while small bird motifs recur in the station’s detail spaces.', source: architecture.o52.id },
    { text: 'The platform wall treatment places birds among reeds, extending the moon-and-habitat theme into the passenger environment.', source: architecture.o52.id },
    { text: 'The department describes these treatments as art integrated into the architectural finish, so the page is evidence of a design programme rather than a complete artwork inventory.', source: architecture.o52.id },
    { text: 'The broader station guidance says a station should be recognisable in its own right, fit its surroundings and remain durable as a modern work over time.', source: dortsStationArchitectureGeneral.id },
    { text: 'O52’s source material accordingly connects a local wetland vocabulary with the practical surfaces and circulation elements of an underground station.', source: architecture.o52.id },
    { text: 'The fetched primary records do not name an individual architect, civil-contract award value, final account or legal rename for this station.', source: architecture.o52.id },
    { text: 'A complete current record of commissioned artwork and any gazette behind a historical code or name would be needed to replace those TBC fields.', source: dortsStationArchitectureGeneral.id },
  ],
  O53: [
    { text: 'DORTS gives O53 the theme By the Water and uses blue and green to distinguish its version of the common Luzhou branch vocabulary.', source: architecture.o53.id },
    { text: 'The page treats water as a long historical memory and asks the station to animate the everyday relationship between egrets and water rather than reproduce a literal landscape.', source: architecture.o53.id },
    { text: 'Water-flow effects are carried by the lobby floor, handrails, waiting-area surfaces and track-side walls, so the theme follows the passenger route through the station.', source: architecture.o53.id },
    { text: 'The waterbird reference also appears in the ventilation shaft, whose nest-like form links a necessary mechanical structure to the branch’s ecological imagery.', source: architecture.o53.id },
    { text: 'This is a different emphasis from O51’s sandbar and O52’s reeds: O53’s source material makes movement and reflection the dominant visual tools.', source: architecture.o53.id },
    { text: 'The department’s station-design guidance says that individual character should coexist with a clear network image, which explains why the three branch stations can vary without losing their shared egret language.', source: dortsStationArchitectureGeneral.id },
    { text: 'The same guidance asks designers to integrate stations with nearby buildings and to use modern materials and construction methods to express regional meaning.', source: dortsStationArchitectureGeneral.id },
    { text: 'O53’s documented lightness is therefore a response to its street setting as well as a visual choice: the above-ground elements are meant to remain clear without becoming heavy landmarks.', source: architecture.o53.id },
    { text: 'The full page documents the design concept and its material applications, but it does not give a separate title-by-artist-by-cost artwork register for the station.', source: architecture.o53.id },
    { text: 'The distinction is retained here because integrated wall, floor and ventilation treatments are not automatically equivalent to a separately procured public-art commission.', source: architecture.o53.id },
    { text: 'The fetched primary records do not name an individual architect, civil-contract value, final account or legal rename for O53.', source: architecture.o53.id },
    { text: 'Those missing records remain TBC pending a contract file, completion account, gazette or complete current artwork register.', source: dortsStationArchitectureGeneral.id },
  ...
  O54: [
    { text: 'The station’s public-art theme is Dancing in the Wind, so the source connects the built station, the possible plaza and the commissioned work through one outdoor-to-indoor concept.', source: architecture.o54.id },
    { text: 'The branch-wide entrance language still uses white egrets, but O54 changes the local emphasis from habitat or water to air and movement.', source: architecture.o54.id },
    { text: 'The ventilation-shaft design draws on two egrets turning back to preen, making a functional opening part of the station’s wind vocabulary.', source: architecture.o54.id },
    { text: 'The page describes the station’s public-space treatment as an attempt to make wind and sunlight present below ground rather than leaving the landscape idea at the plaza boundary.', source: architecture.o54.id },
    { text: 'A large skylight is the principal daylight device, while the source also records fibre-optic light tracing a romantic band across the floor at night.', source: architecture.o54.id },
    { text: 'The purple-red theme colour appears at the entrances, and double curves on the floor express the continuing movement of wind.', source: architecture.o54.id },
    { text: 'Bird imagery returns on the glass railings and platform-side walls, so the commissioned and architectural elements remain visibly related without being treated as the same object.', source: architecture.o54.id },
    { text: 'The DORTS architecture guidance asks stations to have a durable modern identity, to respond to their surroundings and to interpret tradition through contemporary materials and methods.', source: dortsStationArchitectureGeneral.id },
    { text: 'O54’s page shows that principle operating at several scales: the street entrance, the ventilation structure, the underground daylight system and the proposed public square.', source: architecture.o54.id },
    { text: 'The full source identifies Ray King’s coated-glass work and the invited-competition route, but it does not publish a year or cost in the station record.', source: architecture.o54.id },
    { text: 'The fetched primary records also do not name an individual architect, civil-contract value, final account or legal rename for O54.', source: architecture.o54.id },
    { text: 'A complete current artwork register and the relevant procurement or gazette records would be needed to replace those remaining TBC fields.', source: dortsStationArchitectureGeneral.id },
...
const proseSourceEntries: Record<string, Source[]> = {
  ...
  O51: [architecture.o51, dortsStationArchitectureGeneral],
  O52: [architecture.o52, dortsStationArchitectureGeneral],
  O53: [architecture.o53, dortsStationArchitectureGeneral],
  O54: [architecture.o54, dortsStationArchitectureGeneral],
}
