# 雙溪區公所 / Shuangxi District Office — operator research
...
Researched 22 August 2026. Subject: 雙溪區公所 / Shuangxi District Office; TDX record bus-operator-shuangxidistrictoffice-zayq8b.

## What is established

### TDX record identity and route scope

- **Claim.** The committed TDX bus pull identifies bus-operator-shuangxidistrictoffice-zayq8b as the Shuangxi District Office / 雙溪區公所 record in the NewTaipei source set, with operator code ShuangxiDistrictOffice, operator number 1803, phone (02)2493-1111 and URL http://www.shuangxi.ntpc.gov.tw.
...
- **Original-language sentence.** The repository's TDX operator row contains `id: bus-operator-shuangxidistrictoffice-zayq8b`, `names.zh_tw: 雙溪區公所`, `operatorId: 16249`, `operatorNo: 1803`, `phone: (02)2493-1111`, and `sourceCities: [NewTaipei]`.

- **Claim.** The committed route pull associates 6 route records with this operator ID; the content page renders those rows from `operatorIds` rather than hand-listing a route roster.
- **Source.** TDX bus operator and route pull / `titleOriginal`: 交通部運輸資料流通服務平臺
- **URL.** https://tdx.transportdata.tw/
...
- **Original-language sentence.** The committed route data contains 6 route objects whose `operatorIds` array includes `bus-operator-shuangxidistrictoffice-zayq8b`.
...
2. **Route rows versus service families.** The TDX count is 6 rows. Those rows may represent route, branch, direction or departure variants; no claim is made that the count equals a distinct route-family count.
