# 淡水區公所 / Tamsui District Office — operator research
...
Researched 22 August 2026. Subject: 淡水區公所 / Tamsui District Office; TDX record bus-operator-tamsuidistrictoffice-105inwu.

## What is established

### TDX record identity and route scope

- **Claim.** The committed TDX bus pull identifies bus-operator-tamsuidistrictoffice-105inwu as the Tamsui District Office / 淡水區公所 record in the NewTaipei source set, with operator code TamsuiDistrictOffice, operator number 1108, phone (02)2622-1020 and URL http://www.tamsui.ntpc.gov.tw.
...
- **Original-language sentence.** The repository's TDX operator row contains `id: bus-operator-tamsuidistrictoffice-105inwu`, `names.zh_tw: 淡水區公所`, `operatorId: 16228`, `operatorNo: 1108`, `phone: (02)2622-1020`, and `sourceCities: [NewTaipei]`.

- **Claim.** The committed route pull associates 8 route records with this operator ID; the content page renders those rows from `operatorIds` rather than hand-listing a route roster.
- **Source.** TDX bus operator and route pull / `titleOriginal`: 交通部運輸資料流通服務平臺
- **URL.** https://tdx.transportdata.tw/
...
- **Original-language sentence.** The committed route data contains 8 route objects whose `operatorIds` array includes `bus-operator-tamsuidistrictoffice-105inwu`.
...
2. **Route rows versus service families.** The TDX count is 8 rows. Those rows may represent route, branch, direction or departure variants; no claim is made that the count equals a distinct route-family count.
