# 林口區公所 / Linkou District Office — operator research
...
Researched 22 August 2026. Subject: 林口區公所 / Linkou District Office; TDX record bus-operator-linkoudistrictoffice-y7olk0.

## What is established

### TDX record identity and route scope

- **Claim.** The committed TDX bus pull identifies bus-operator-linkoudistrictoffice-y7olk0 as the Linkou District Office / 林口區公所 record in the NewTaipei source set, with operator code LinkouDistrictOffice, operator number 0812, phone (02)2603-3111 and URL http://www.linkou.ntpc.gov.tw.
...
- **Original-language sentence.** The repository's TDX operator row contains `id: bus-operator-linkoudistrictoffice-y7olk0`, `names.zh_tw: 林口區公所`, `operatorId: 16235`, `operatorNo: 0812`, `phone: (02)2603-3111`, and `sourceCities: [NewTaipei]`.

- **Claim.** The committed route pull associates 7 route records with this operator ID; the content page renders those rows from `operatorIds` rather than hand-listing a route roster.
- **Source.** TDX bus operator and route pull / `titleOriginal`: 交通部運輸資料流通服務平臺
- **URL.** https://tdx.transportdata.tw/
...
- **Original-language sentence.** The committed route data contains 7 route objects whose `operatorIds` array includes `bus-operator-linkoudistrictoffice-y7olk0`.
...
2. **Route rows versus service families.** The TDX count is 7 rows. Those rows may represent route, branch, direction or departure variants; no claim is made that the count equals a distinct route-family count.
