# 樹林區公所 / Shulin District Office — operator research
...
Researched 22 August 2026. Subject: 樹林區公所 / Shulin District Office; TDX record bus-operator-shulindistrictoffice-xd1idi.

## What is established

### TDX record identity and route scope

- **Claim.** The committed TDX bus pull identifies bus-operator-shulindistrictoffice-xd1idi as the Shulin District Office / 樹林區公所 record in the NewTaipei source set, with operator code ShulinDistrictOffice, operator number 1603, phone (02)2681-2106 and URL http://www.shulin.ntpc.gov.tw.
...
- **Original-language sentence.** The repository's TDX operator row contains `id: bus-operator-shulindistrictoffice-xd1idi`, `names.zh_tw: 樹林區公所`, `operatorId: 16242`, `operatorNo: 1603`, `phone: (02)2681-2106`, and `sourceCities: [NewTaipei]`.

- **Claim.** The committed route pull associates 1 route records with this operator ID; the content page renders those rows from `operatorIds` rather than hand-listing a route roster.
- **Source.** TDX bus operator and route pull / `titleOriginal`: 交通部運輸資料流通服務平臺
- **URL.** https://tdx.transportdata.tw/
...
- **Original-language sentence.** The committed route data contains 1 route objects whose `operatorIds` array includes `bus-operator-shulindistrictoffice-xd1idi`.
...
2. **Route rows versus service families.** The TDX count is 1 rows. Those rows may represent route, branch, direction or departure variants; no claim is made that the count equals a distinct route-family count.
