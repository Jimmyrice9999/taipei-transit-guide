# 新莊區公所 / Xinzhuang District Office — operator research
...
Researched 22 August 2026. Subject: 新莊區公所 / Xinzhuang District Office; TDX record bus-operator-xinzhuangdistrictoffice-105ltet.

## What is established

### TDX record identity and route scope

- **Claim.** The committed TDX bus pull identifies bus-operator-xinzhuangdistrictoffice-105ltet as the Xinzhuang District Office / 新莊區公所 record in the NewTaipei source set, with operator code XinzhuangDistrictOffice, operator number 1315, phone (02)2992-9891#410 and URL http://www.xinzhuang.ntpc.gov.tw.
...
- **Original-language sentence.** The repository's TDX operator row contains `id: bus-operator-xinzhuangdistrictoffice-105ltet`, `names.zh_tw: 新莊區公所`, `operatorId: 16232`, `operatorNo: 1315`, `phone: (02)2992-9891#410`, and `sourceCities: [NewTaipei]`.

- **Claim.** The committed route pull associates 5 route records with this operator ID; the content page renders those rows from `operatorIds` rather than hand-listing a route roster.
- **Source.** TDX bus operator and route pull / `titleOriginal`: 交通部運輸資料流通服務平臺
- **URL.** https://tdx.transportdata.tw/
...
- **Original-language sentence.** The committed route data contains 5 route objects whose `operatorIds` array includes `bus-operator-xinzhuangdistrictoffice-105ltet`.
...
2. **Route rows versus service families.** The TDX count is 5 rows. Those rows may represent route, branch, direction or departure variants; no claim is made that the count equals a distinct route-family count.
