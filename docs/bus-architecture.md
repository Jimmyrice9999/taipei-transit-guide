...
## Current implementation and New Taipei strategy — Run 50, 19 August 2026

The proposal above is retained as the original design record. The bus layer is
now live: 1,051 normalized TDX route records are committed under
`data/tdx/bus/`, route overlays use
`content/bus/routes/<group>/<slug>.md`, and the static route templates are
`app/bus/routes/[group]/page.tsx` and
`app/bus/routes/[group]/[slug]/page.tsx`. The current built-route count and
group list are derived from route overlays and `_index.md` files; route pages
are never addressed by an unscoped number.

New Taipei is a municipality-scoped canonical group. Its 562 currently
unbuilt identities are grouped on `/bus/routes/new-taipei/` by the exact route
type datasets published by the New Taipei Department of Transportation, not by
district or current operator: general bus (217), New Bus community service
(207), rapid bus (57), jump-frog commuter bus (70), light-rail feeder (5), MRT
pioneer bus (2), and event shuttle (4). Those seven groups are the stable
subgroup keys in `lib/bus/new-taipei.ts`; each is collapsed by default on the
municipality index. A route remains at one canonical URL under
`new-taipei`, so a Taipei route with the same number cannot overwrite it.

The New Taipei source hierarchy is: (1) TDX for normalized identity,
municipality, operators, variants, stops, sequences, shapes and geometry-only
rail joins (`https://tdx.transportdata.tw/`); (2) the Department of
Transportation's route-type datasets and current route list
(`https://data.ntpc.gov.tw/datasets/1016bb4c-e8eb-4113-a2be-657fb51a1558` and
`https://data.ntpc.gov.tw/api/datasets/0ee4e6bf-cee6-4ec8-8fe1-71f544015127/json?size=1000`);
(3) dated New Taipei government notices, gazettes and official operator pages
for history and unusual service rules. The transport-management landing page
(`https://www.traffic.ntpc.gov.tw/home.jsp?id=f06672ba2aadbd3c`) is the source
map for the official rapid, jump-frog and low-floor resources. Taipei's eBus
catalogue remains a lead for New Taipei only; it is not used as authority for
New Taipei-specific category or service claims.

`data/ntpc/bus-route-service.json` is the fetched static snapshot of the
current New Taipei route list joined to TDX by exact Traditional-Chinese route
name. It supplies published endpoints, distance, first/last service, headway
fields and fare where a current row exists. Forty of the 562 identities have
no exact current service row in the fetched dataset; their service fields stay
TBC. No related route is used as a proxy. The file is regenerated by
`npm run ntpc:bus` and is not a substitute for the TDX stop or geometry layer.
