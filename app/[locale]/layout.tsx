...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isLocale(currentLocale)) notFound()
  ...
  return (
    /*
     * The locale segment is static, so the document language is known while
     * exporting each page. Individual English synthesis remains explicitly
     * English where no committed Traditional-Chinese equivalent exists.
     */
    <html lang={currentLocale} className={`${zilla.variable} ${inter.variable} ${plexMono.variable}`}>
      <head>
        <style dangerouslySetInnerHTML={{ __html: HAN_FONT_FACE }} />
        <JsonLd data={websiteSchema(currentLocale as Locale)} />
      </head>
      <body>
        <LocaleProvider locale={currentLocale as Locale}>
          ...
          {/*
            ...
            a plain block and the rail is `display: none`.
          */}
